...
(function() {
  ...
  function setupSuppAccordion() {
    ...
    if (!btn || !list) return;
    btn.addEventListener("click", (e) => { try{e.preventDefault();e.stopPropagation();}catch(_){} list.classList.toggle("hidden"); btn.textContent = "Xem từng người"; });
}
  ...
  if (!root) return;
  let rafId = null;
  const schedule = ()=>{ if(rafId) return; rafId = requestAnimationFrame(()=>{ rafId=null; refreshUI(); }); };
  const obs = new MutationObserver((mutations)=>{ if (!mutations.some(m=>m.type==="childList")) return; schedule(); });
