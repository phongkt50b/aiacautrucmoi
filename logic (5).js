...
if (suppListEl) {
  ...
  // Dòng NĐBH chính (tổng bổ sung + MDP3 nếu gán cho chính)
  ...
  const mainSupp = Number((personFees[mainId]?.supp)||0);
  const mainSum = mainSupp + ((mdp3SelectedId===mainId)?mdp3Fee:0);
  const mainRow = document.createElement('div');
  mainRow.className = 'flex justify-between items-center py-1 text-sm';
  mainRow.innerHTML = `<span>${sanitizeHtml(mainName)}</span><span class="font-semibold">${fmt(mainSum)}</span>`;
  ...
  document.querySelectorAll('#supplementary-insured-container .person-container').forEach((cont, idx) => {
    const pid = cont.id;
    const info = getCustomerInfo(cont,false);
    const name = info.name || `NĐBH bổ sung ${idx+1}`;
    const supp = Number((personFees[pid]?.supp)||0) + ((mdp3SelectedId===pid)?mdp3Fee:0);
    const row = document.createElement('div');
    row.className = 'flex justify-between items-center py-1 text-sm';
    row.innerHTML = `<span>${sanitizeHtml(name)}</span><span class="font-semibold">${fmt(supp)}</span>`;
    suppListEl.appendChild(row);
  });

  // Nếu chọn MDP3 = "Người khác" thì thêm dòng riêng
  if (mdp3Fee>0 && mdp3SelectedId==='other') {
    const row = document.createElement('div');
    row.className = 'flex justify-between items-center py-1 text-sm';
    row.innerHTML = `<span>Miễn đóng phí 3.0 (Người khác)</span><span class="font-semibold">${fmt(mdp3Fee)}</span>`;
    suppListEl.appendChild(row);
  }
}
...
function setupSuppAccordion() {
    ...
    btn.addEventListener('click', (e) => {
        ...
        // Giữ nhãn cố định
        btn.textContent = 'Xem từng người';
    });
}
);
  }
  ...
  function refreshUI() {
    updateBadge();
    hideZeroLines();
    updatePeriodBreakdown(); hideZeroLines();
  }
...
function setupObservers() {
    ...
    const schedule = () => {
        if (raf) return;
        raf = requestAnimationFrame(() => { raf = 0; try { refreshUI(); } catch(_){} });
    };
