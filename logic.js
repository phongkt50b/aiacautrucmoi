...
function removeCompatStuff() {
  const ids = ['compat-mode-btn', 'compat-mode', 'compat-section', 'compat-toggle'];
  ids.forEach(id => { const el = document.getElementById(id); if (el) el.remove(); });
  const shim = document.getElementById('compat-shim');
  if (shim) shim.remove(); // bạn yêu cầu bỏ phần tương thích cũ
}
...
function _round1k(n){ n=Number(n)||0; return Math.round(n/1000)*1000; }
...
function generateSummaryTable() {
  ...
  const label = freq === 'half' ? 'nửa năm' : (freq === 'quarter' ? 'quý' : 'năm');
  const periods = freq === 'half' ? 2 : (freq === 'quarter' ? 4 : 1);
  const factor = freq === 'half' ? 1.02 : (freq === 'quarter' ? 1.04 : 1);

  // lấy tổng phí năm hiện tại từ UI
  const totals = (typeof computeYearTotals === 'function') ? computeYearTotals() : {main:0, extra:0, suppAll:0, mainPlusExtra:0};
  let mainAnnual = Number(totals.main||0);
  let extraAnnual = Number(totals.extra||0);
  let suppAnnual = Number(totals.suppAll||0);

  // phân bổ theo người từ cache
  const personFees = (window.personFees || {});
  let mainSupp = Number(personFees['main-person-container']?.supp || 0);
  // cộng tất cả NĐBH bổ sung
  let suppPeople = Array.from(document.querySelectorAll('#supplementary-insured-container .person-container')).map(cont => {
    const id = cont.id;
    const name = cont.querySelector('.name-input')?.value?.trim() || 'NĐBH bổ sung';
    const supp = Number(personFees[id]?.supp || 0);
    return { id, name, supp };
  });

  // MDP 3.0: cộng đúng người nếu được gán
  try {
    if (window.MDP3 && document.getElementById('mdp3-enable')?.checked) {
      const selId = window.MDP3.getSelectedId && window.MDP3.getSelectedId();
      const fee = Number(window.MDP3.getPremium ? (window.MDP3.getPremium()||0) : 0);
      if (fee > 0) {
        if (selId === 'main-person-container') {
          mainSupp += fee;
        } else if (selId && suppPeople.some(p => p.id === selId)) {
          const idx = suppPeople.findIndex(p => p.id === selId);
          if (idx >= 0) suppPeople[idx].supp += fee;
        } else {
          // "Người khác": không tính vào từng người, vẫn tính vào tổng bổ sung
        }
        // Đảm bảo tổng bổ sung năm khớp với chi tiết theo người
        const sumPeople = mainSupp + suppPeople.reduce((a,b)=>a + Number(b.supp||0), 0);
        if (sumPeople > 0) suppAnnual = sumPeople;
      }
    }
  } catch {}

  const totalAnnual = mainAnnual + extraAnnual + suppAnnual;

  // TÍNH THEO KỲ (nếu nửa năm/quý)
  const perMainExtra = (periods === 1) ? (mainAnnual + extraAnnual) : _round1k((mainAnnual + extraAnnual) / periods);
  const perSupp = (periods === 1) ? suppAnnual : _round1k((suppAnnual * factor) / periods);
  const perTotal = perMainExtra + perSupp;
  const quyNam = (periods === 1) ? totalAnnual : (perTotal * periods);
  const chenh = (periods === 1) ? 0 : (quyNam - totalAnnual);
  ...
  let html = `
    <div class="space-y-4">
      <div>
        <div class="text-lg font-semibold mb-2">Tóm tắt phí (theo năm)</div>
        <div class="grid grid-cols-2 gap-2 text-sm">
          <div class="flex justify-between"><span>Phí sản phẩm chính (năm)</span><span class="font-semibold">${fmt(mainAnnual)}</span></div>
          <div class="flex justify-between"><span>Phí đóng thêm (năm)</span><span class="font-semibold">${fmt(extraAnnual)}</span></div>
          <div class="flex justify-between"><span>Phí sản phẩm bổ sung (năm)</span><span class="font-semibold">${fmt(suppAnnual)}</span></div>
          <div class="flex justify-between col-span-2 border-t pt-2"><span class="font-semibold">Tổng tất cả (năm)</span><span class="font-bold">${fmt(totalAnnual)}</span></div>
        </div>
      </div>
  `;

  // Danh sách từng người cho mục bổ sung
  html += `
      <div>
        <div class="text-base font-semibold mb-2">Phí sản phẩm bổ sung · Xem từng người</div>
        <div class="space-y-1 text-sm">
          <div class="flex justify-between"><span>${sanitizeHtml((getMainPersonInfo()?.name)||'NĐBH chính')}</span><span class="font-semibold">${fmt(mainSupp)}</span></div>
          ${suppPeople.map(p => `<div class="flex justify-between"><span>${sanitizeHtml(p.name)}</span><span class="font-semibold">${fmt(p.supp||0)}</span></div>`).join('')}
        </div>
      </div>
  `;

  // Khối theo kỳ (chỉ khi chọn nửa năm/quý)
  if (periods > 1) {
    html += `
      <div>
        <div class="text-lg font-semibold mb-2">Phí theo ${label}</div>
        <div class="grid grid-cols-2 gap-2 text-sm">
          <div class="flex justify-between"><span>Phí chính + đóng thêm theo ${label}</span><span class="font-semibold">${fmt(perMainExtra)}</span></div>
          <div class="flex justify-between"><span>Phí sản phẩm bổ sung theo ${label}</span><span class="font-semibold">${fmt(perSupp)}</span></div>
          <div class="flex justify-between col-span-2 border-t pt-2"><span class="font-semibold">Tổng theo ${label}</span><span class="font-bold">${fmt(perTotal)}</span></div>
          <div class="flex justify-between"><span>Quy năm</span><span class="font-semibold">${fmt(quyNam)}</span></div>
          <div class="flex justify-between"><span>Chênh lệch nếu đóng 1 năm</span><span class="font-semibold">${fmt(chenh)}</span></div>
        </div>
      </div>
    `;
  }
