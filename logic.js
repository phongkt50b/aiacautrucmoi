...
function generateSummaryTable() {
  ...
  try {
    ...
    const mainPersonContainer = document.getElementById('main-person-container');
    const mainInfo = getCustomerInfo(mainPersonContainer, true);
    const mainProduct = mainInfo.mainProduct;

    if (isNaN(targetAge) || targetAge <= mainInfo.age) {
      throw new Error("Vui lòng nhập độ tuổi mục tiêu hợp lệ, lớn hơn tuổi hiện tại của NĐBH chính.");
    }

    // TTA bắt buộc có Bùng Gia Lực
    if (mainProduct === 'TRON_TAM_AN') {
      ...
      const healthSclPremium = calculateHealthSclPremium(mainInfo, mainSuppContainer);
      if (!healthSclCheckbox?.checked || healthSclPremium === 0) {
        throw new Error('Trọn Tâm An bắt buộc tham gia kèm Sức Khỏe Bùng Gia Lực có phí hợp lệ.');
      }
    }

    // Năm đóng phí (paymentTerm)
    let paymentTerm = 999;
    const paymentTermInput = document.getElementById('payment-term');
    ...
    if (['PUL_TRON_DOI','PUL_5_NAM','PUL_15_NAM','KHOE_BINH_AN','VUNG_TUONG_LAI'].includes(mainProduct) &&
      ...
      throw new Error(`Không hợp lệ, từ ${mainInfo.age + paymentTerm - 1} đến 100`);
    }

    // Thu thập NĐBH bổ sung
    const suppPersons = [];
    document.querySelectorAll('.person-container').forEach(p => {
      if (p.id !== 'main-person-container') {
    ...
    // Lấy cấu phần phí hiện tại
    const baseMain0 = calculateMainPremium(mainInfo);              // phí SP chính ở tuổi hiện tại
    const extra0 = getExtraPremiumValue();                         // đóng thêm (năm)
    const mainSuppCont = document.querySelector('#main-supp-container .supplementary-products-container');
    const freq = // Header bảng
    let html = `<table class="w-full text-left border-collapse">
      <thead class="bg-gray-100">
        <tr>
          <th class="p-2 border">Năm HĐ</th>
          <th class="p-2 border">Tuổi NĐBH chính<br>(${sanitizeHtml(mainInfo.name)})</th>
          <th class="p-2 border">Phí chính</th>
          <th class="p-2 border">Phí đóng thêm</th>
          <th class="p-2 border">Phí bổ sung (${sanitizeHtml(mainInfo.name)})</th>`;
    suppPersons.forEach(p => {
      html += `<th class="p-2 border">Phí bổ sung (${sanitizeHtml(p.name)})</th>`;
    });
    html += `<th class="p-2 border">Tổng cộng</th>`;
    if (freq !== 'year') html += `<th class="p-2 border">Chênh lệch (so với năm)</th>`;
    html += `</tr></thead><tbody>`;

    let sumMain = 0, sumExtra = 0, sumSuppMain = 0, sumTotal = 0, sumDiff = 0;

    // Hạn mức tổng STBH hỗ trợ viện phí theo PHÍ CHÍNH (không gồm đóng thêm)
    const totalMaxSupport = Math.floor(baseMain0 / 4000000) * 100000;

    for (let i = 0; (mainInfo.age + i) <= targetAge; i++) {
      const ageNow = mainInfo.age + i;
      const yearNo = i + 1;

      const mainBaseYear = (yearNo <= paymentTerm) ? baseMain0 : 0;
      const extraYear    = (yearNo <= paymentTerm) ? extra0     : 0;

      // Tính SPBS từng người (theo tuổi của năm đó)
      let totalHospitalSupportStbh = 0;
      let suppMainYear = 0;
      if (mainSuppCont) {
        suppMainYear += calculateHealthSclPremium({ ...mainInfo, age: ageNow }, mainSuppCont, ageNow);
        suppMainYear += calculateBhnPremium({ ...mainInfo, age: ageNow }, mainSuppCont, ageNow);
        suppMainYear += calculateAccidentPremium({ ...mainInfo, age: ageNow }, mainSuppCont, ageNow);
        suppMainYear += calculateHospitalSupportPremium({ ...mainInfo, age: ageNow }, baseMain0, mainSuppCont, totalHospitalSupportStbh, ageNow);
        const stbhHS = parseFormattedNumber(mainSuppCont.querySelector('.hospital-support-stbh')?.value || '0');
        if (mainSuppCont.querySelector('.hospital-support-checkbox')?.checked && stbhHS > 0) totalHospitalSupportStbh += stbhHS;
      }

      const suppEach = suppPersons.map(person => {
        ...
        if (cont) {
          ...
          s += calculateHospitalSupportPremium({ ...person, age: person.age + i }, baseMain0, cont, totalHospitalSupportStbh, person.age + i);
          const stbhHS = parseFormattedNumber(cont.querySelector('.hospital-support-stbh')?.value || '0');
          if (cont.querySelector('.hospital-support-checkbox')?.checked && stbhHS > 0) totalHospitalSupportStbh += stbhHS;
        }
        return s;
      });

      // MDP3 như phần tóm tắt (đến tuổi 65 của người được chọn); nếu vượt, phí = 0
      const mdp3FeeAll = (window.MDP3 ? MDP3.getPremium() : 0) || 0;
      let mdp3FeeYear = mdp3FeeAll;
      try {
        const mdp3SelId = (window.MDP3 && typeof MDP3.getSelectedId==='function') ? MDP3.getSelectedId() : null;
        if (mdp3SelId && mdp3SelId !== 'other') {
          // find that person to check age limit 65
          const pEl = document.getElementById(mdp3SelId);
          const pInfo = pEl ? getCustomerInfo(pEl, false) : null;
          if (pInfo && (pInfo.age + i) > 65) mdp3FeeYear = 0;
        } else {
          // if 'other', apply limit 65 to main insured
          if ((mainInfo.age + i) > 65) mdp3FeeYear = 0;
        }
      } catch(e){ /* no-op */ }

      let suppAllYear = suppMainYear + suppEach.reduce((s,v)=>s+v,0) + mdp3FeeYear;

      // Áp dụng kỳ đóng phí cho PHẦN BỔ SUNG nếu chọn quý/nửa
      const periods = (freq === 'half') ? 2 : (freq === 'quarter' ? 4 : 1);
      const factor = (freq === 'half') ? 1.02 : (freq === 'quarter' ? 1.04 : 1.0);

      // Main+Extra theo kỳ = chia đều, làm tròn xuống 1.000/ kỳ
      const perMainExtra = periods===1 ? (mainBaseYear + extraYear) : Math.floor((mainBaseYear + extraYear) / periods / 1000) * 1000;
      // Supplementary theo kỳ = (năm/1000 * factor / periods) roundDown * 1000
      const perSupp = periods===1 ? suppAllYear : Math.floor((suppAllYear / 1000 * factor / periods)) * 1000;

      const shownSuppAnnual = perSupp * periods;
      const shownTotalAnnual = (perMainExtra + perSupp) * periods;
      const diff = shownTotalAnnual - (mainBaseYear + extraYear + suppAllYear);

      html += `<tr>
        <td class="p-2 border text-center">${yearNo}</td>
        <td class="p-2 border text-center">${ageNow}</td>
        <td class="p-2 border text-right">${formatCurrency(mainBaseYear)}</td>
        <td class="p-2 border text-right">${formatCurrency(extraYear)}</td>
        <td class="p-2 border text-right">${formatCurrency(shownSuppAnnual - suppEach.reduce((s,v)=>s+v,0) - mdp3FeeYear)}</td>`;
      // Tách từng NĐBH bổ sung
      suppEach.forEach(su => {
        let sAdj = su;
        if (periods!==1) sAdj = (Math.floor((su / 1000 * factor / periods)) * 1000) * periods;
        html += `<td class="p-2 border text-right">${formatCurrency(sAdj)}</td>`;
      });

      html += `<td class="p-2 border text-right font-semibold">${formatCurrency(shownTotalAnnual)}</td>`;
      if (periods!==1) html += `<td class="p-2 border text-right">${formatCurrency(diff)}</td>`;
      html += `</tr>`;

      sumMain += mainBaseYear;
      sumExtra += extraYear;
      sumSuppMain += shownSuppAnnual - suppEach.reduce((s,v)=>s+v,0) - mdp3FeeYear;
      sumTotal += shownTotalAnnual;
      sumDiff += diff;

      if (totalHospitalSupportStbh > totalMaxSupport) {
        throw new Error(`Tổng STBH Hỗ trợ viện phí vượt hạn mức chung: ${formatCurrency(totalMaxSupport, 'đ/ngày')}.`);
      }
    }
    ...
    html += `<tr class="bg-gray-200 font-bold">
      ...
      <td class="p-2 border text-right">${formatCurrency(sumMain)}</td>
      <td class="p-2 border text-right">${formatCurrency(sumExtra)}</td>
      <td class="p-2 border text-right">${formatCurrency(sumSuppMain)}</td>`;
    suppPersons.forEach(()=> html += `<td class="p-2 border text-right">—</td>`);
    html += `<td class="p-2 border text-right">${formatCurrency(sumTotal)}</td>`;
    if (freq !== 'year') html += `<td class="p-2 border text-right">${formatCurrency(sumDiff)}</td>`;
    html += `</tr></tbody></table>`;

    html += `<div class="mt-4 text-sm font-semibold text-gray-700">
...
window.MDP3 = (function () {
    ...
    return { getSelectedId,  init, renderSection, renderSelect, getPremium, reset, resetIfEnabled };
})();
...
// === Section 6: minimal hook without touching Sections 1–5 ===
(function(){
  function floor1000(n){ n = Number(n)||0; return Math.floor(n/1000)*1000; }
  function formatVND(n){ try{ return formatCurrency(n); }catch(e){ return (Number(n)||0).toLocaleString('vi-VN') + ' VNĐ'; } }
  function getNameFor(container){
    try{ return getCustomerInfo(container, container.id==='main-person-container').name || 'NĐBH bổ sung'; }catch(e){ return 'NĐBH bổ sung'; }
  }

  function collectBreakdown(){
    const pf = window.personFees || {};
    const main = pf['main-person-container'] || { main:0, mainBase:0, supp:0, total:0 };
    // Danh sách NĐBH bổ sung
    const list = [];
    document.querySelectorAll('#supplementary-insured-container .person-container').forEach(c => {
      const fee = pf[c.id]?.supp || 0;
      list.push({ id: c.id, name: getNameFor(c), supp: fee });
    });

    // MDP3
    let mdp3Fee = 0, mdp3Target = null;
    try {
      mdp3Fee = (window.MDP3 ? MDP3.getPremium() : 0) || 0;
      mdp3Target = document.getElementById('mdp3-person-select')?.value || null; // 'other' | personId | ''
    } catch(e){ /* ignore */ }

    return { main, list, mdp3Fee, mdp3Target };
  }

  function computePeriod(baseMain, extra, suppTotal, freq){
    const annual = floor1000(baseMain) + floor1000(extra) + floor1000(suppTotal);
    if (freq==='year' || !freq){
      return { perMainExtra: floor1000(baseMain+extra), perSupp: floor1000(suppTotal), perTotal: floor1000(baseMain+extra+suppTotal), yearFromPeriods: annual, diff: 0 };
    }
    const periods = (freq==='half')?2:4;
    const factor = (freq==='half')?1.02:1.04;
    const perMainExtra = floor1000((baseMain+extra)/periods);
    const perSupp = Math.floor((suppTotal/1000*factor/periods))*1000;
    const perTotal = perMainExtra + perSupp;
    const yearFromPeriods = perTotal*periods;
    const diff = yearFromPeriods - annual;
    return { perMainExtra, perSupp, perTotal, yearFromPeriods, diff };
  }

  function renderSection6Using(premiums){
    const freqSel = document.getElementById('payment-frequency');
    const freq = freqSel ? freqSel.value : 'year';

    const { main, list, mdp3Fee, mdp3Target } = collectBreakdown();
    const baseMain = Number(main.mainBase||0);
    const extra = Math.max(0, Number(premiums.mainPremium||0) - baseMain);
    const mainSupp = Number(main.supp||0);
    // phân bổ MDP3: nếu chọn 1 người cụ thể -> cộng vào người đó; nếu 'other' -> gộp chung vào tổng SPBS
    let listAdjusted = list.map(x => ({...x}));
    let mdp3Other = 0;
    if (mdp3Fee>0){
      if (mdp3Target && mdp3Target!=='other'){
        const tgt = listAdjusted.find(p => p.id===mdp3Target);
        if (tgt) tgt.supp += mdp3Fee; else mdp3Other = mdp3Fee;
      } else if (mdp3Target==='other') {
        mdp3Other = mdp3Fee;
      }
    }

    const suppOthers = listAdjusted.reduce((s,p)=>s+(p.supp||0),0);
    const suppTotal = mainSupp + suppOthers + mdp3Other;

    // Dòng 1-4: NĐBH chính
    const mainInsuredTotal = baseMain + extra + mainSupp;
    const el1 = document.getElementById('main-insured-total');
    if (el1){
      el1.textContent = formatVND(mainInsuredTotal);
      const a = document.getElementById('main-insured-main-fee'); if (a) a.textContent = formatVND(baseMain);
      const b = document.getElementById('main-insured-extra-fee'); if (b) b.textContent = formatVND(extra);
      const c = document.getElementById('main-insured-supp-fee');  if (c) c.textContent = formatVND(mainSupp);
    }

    // Dòng 5+: từng NĐBH bổ sung
    const cont = document.getElementById('supp-insured-summaries');
    if (cont){
      cont.innerHTML = '';
      listAdjusted.forEach((p, idx) => {
        const line = document.createElement('div');
        line.className = 'text-sm text-gray-700 flex justify-between border-b py-1';
        line.innerHTML = `<span>Tổng phí của ${p.name} ${idx+1}:</span><span class="font-semibold">${formatVND(p.supp||0)}</span>`;
        cont.appendChild(line);
      });
      if (mdp3Other>0){
        const line = document.createElement('div');
        line.className = 'text-sm text-gray-700 flex justify-between border-b py-1';
        line.innerHTML = `<span>Miễn đóng phí 3.0 (Người khác):</span><span class="font-semibold">${formatVND(mdp3Other)}</span>`;
        cont.appendChild(line);
      }
    }

    // Dòng 6-9: Tóm tắt tổng
    const sumMainEl = document.getElementById('summary-main-fee'); if (sumMainEl) sumMainEl.textContent = formatVND(baseMain);
    const sumExtraEl = document.getElementById('summary-extra-fee'); if (sumExtraEl) sumExtraEl.textContent = formatVND(extra);
    const sumSuppEl  = document.getElementById('summary-supp-fee');  if (sumSuppEl)  sumSuppEl.textContent  = formatVND(suppTotal);
    const sumTotalEl = document.getElementById('summary-total');     if (sumTotalEl) sumTotalEl.textContent = formatVND(baseMain + extra + suppTotal);

    // Dòng 10: Kỳ đóng phí
    const box = document.getElementById('frequency-breakdown');
    if (box){
      if (freq==='year'){ box.classList.add('hidden'); }
      else {
        const bd = computePeriod(baseMain, extra, suppTotal, freq);
        const me = document.getElementById('freq-main-plus-extra'); if (me) me.textContent = formatVND(bd.perMainExtra);
        const ss = document.getElementById('freq-supp-total'); if (ss) ss.textContent = formatVND(bd.perSupp);
        const pt = document.getElementById('freq-total-period'); if (pt) pt.textContent = formatVND(bd.perTotal);
        const ty = document.getElementById('freq-total-year'); if (ty) ty.textContent = formatVND(bd.yearFromPeriods);
        const df = document.getElementById('freq-diff'); if (df) df.textContent = formatVND(bd.diff);
        box.classList.remove('hidden');
      }
    }
  }

  // Wrap updateSummaryUI to render Section 6 right after it updates Section 1–5 results
  const __updateSummaryUI__ = window.updateSummaryUI;
  window._lastPremiumsForSec6 = null;
  window.updateSummaryUI = function(premiums){
    __updateSummaryUI__(premiums);
    window._lastPremiumsForSec6 = premiums;
    try{ renderSection6Using(premiums); }catch(e){ /* ignore */ }
  };

  // Re-render when user changes payment frequency
  document.getElementById('payment-frequency')?.addEventListener('change', function(){
    if (window._lastPremiumsForSec6) renderSection6Using(window._lastPremiumsForSec6);
  });
})();
