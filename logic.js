...
/* =======================
 * Compact UI Enhancer (Approach B)
 * - Non-invasive: keeps all original logic intact.
 * - Only controls display (hide zero rows, accordion, badges, breakdown visibility).
 * ======================= */
(function() {
  ...
  const formatVND = (n) => {
    try { return n.toLocaleString("vi-VN") + " VNĐ"; } catch(e) { return (n+"").replace(/\B(?=(\d{3})+(?!\d))/g, ".") + " VNĐ"; }
  };
  ...
  function hideZeroLines() {
    ...
    pairs.forEach(([id]) => {
      const el = $$(id);
      if (!el) return;
      const val = parseVND(el.textContent);
      const row = el.closest("li, div");
      if (row) row.classList.toggle("hidden", val === 0);
    });
    // If all three of main-insured are hidden, keep container spacing tidy
    const container = $$("#main-insured-summary");
    if (container) {
      const visible = $$$("li:not(.hidden)", container).length || $$$("> .text-sm > div:not(.hidden)", container).length;
  ...
  function toggleBreakdownVisibility() {
    ...
    if (diffEl) {
      const v = parseVND(diffEl.textContent);
      diffEl.closest("div").classList.toggle("hidden", !show || v === 0);
    }
  }
