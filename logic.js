...
/* ===============================================================
 * UI Enhancer v2 (Compact card, no legacy compat block)
 * - Keeps original logic intact; only reads numbers from DOM.
 * - Shows period breakdown for Half/Quarter, including diff.
 * - Hides zero rows; toggles accordion; updates badge.
 * =============================================================== */
(function() {
  ...
  function computeYearTotals() {
    ...
    // Tổng bổ sung (tất cả người) — lấy từ #summary-supp-fee
    const suppAll = toInt(($$("#summary-supp-fee")||{}).textContent);
    const total = toInt(($$("#summary-total")||{}).textContent) || (main + extra + suppAll);
  ...
  function updateBadge() {
    ...
    badge.textContent = map[sel.value] || "Năm";
  }
  ...
  function updatePeriodBreakdown() {
    ...
    const set = (id, val) => { const el = $$(id); if (el) el.textContent = fmt(Math.max(0, Math.round(val))); };
    set("#freq-main-plus-extra", mainExtraPeriod);
    set("#freq-supp-total", suppPeriod);
    set("#freq-total-period", totalPeriod);
    set("#freq-total-year", toYear);
    set("#freq-diff", diff);
  ...
  function hideZeroLines() {
    const ids = ["#main-insured-main-fee","#main-insured-extra-fee","#main-insured-supp-fee","#summary-supp-fee"];
    ids.forEach(id => {
      const el = $$(id);
      if (!el) return;
  ...
  function setupObservers() {
    const root = $$("#results-container") || document.body;
    const obs = new MutationObserver(() => refreshUI());
