...
function updatePaymentFrequencyOptions(baseAnnual){
    ...
    const allowHalf = (Number(baseAnnual)||0) >= 7000000;
    const allowQuarter = (Number(baseAnnual)||0) >= 8000000;
    if (optHalf) { optHalf.disabled = !allowHalf; optHalf.hidden = !allowHalf; }
    if (optQuarter) { optQuarter.disabled = !allowQuarter; optQuarter.hidden = !allowQuarter; }
    if ((sel.value==='half' && !allowHalf) || (sel.value==='quarter' && !allowQuarter)) {
        sel.value = 'year';
    }
}
        ...
        if (window.MDP3) {
            ...
            if (mdp3Fee > 0) {
                ...
                if (selId && selId !== 'other') {
                    if (!window.personFees[selId]) window.personFees[selId] = { main: 0, mainBase: 0, supp: 0, total: 0 };
                    window.personFees[selId].supp += mdp3Fee;
                    window.personFees[selId].total = (window.personFees[selId].main || 0) + (window.personFees[selId].supp || 0);
                }
            }
        }
    ...
    try {
        ...
        (function buildPart1(){
          try {
            ...
            const suppFactor = (freq === 'half') ? 1.02 : (freq === 'quarter' ? 1.04 : 1.0);

            const baseAnnual = calculateMainPremium(mainPersonInfo) || 0;
            const extraAnnual = getExtraPremiumValue() || 0;

            function buildSuppRows(personInfo, containerEl, opts){
              if(!containerEl) return '';
              const nameCell = opts && opts.suppressName ? '' : sanitizeHtml(personInfo.name||'');
              const freqFactor = opts && opts.freqFactor || 1.0;
              const includeMdp3 = opts && opts.includeMdp3;
              const mdp3FeeAnnual = opts && opts.mdp3FeeAnnual || 0;
              const rows = [];
              const startAge = personInfo.age;

              const scl = containerEl.querySelector('.health-scl-section');
              if (scl && scl.querySelector('.health-scl-checkbox')?.checked) {
                ...
                const fee = calculateHealthSclPremium(personInfo, containerEl, startAge);
                const feeAdj = roundDownTo1000((Number(fee)||0) * freqFactor);
                const years = Math.max(0, Math.min(targetAge, 75) - startAge + 1);
                rows.push(`<tr>
                  <td class="p-2 border">${nameCell}</td>
                  ...
                  <td class="p-2 border text-right">${feeAdj>0?formatCurrency(feeAdj):''}</td>
                </tr>`);
              }

              const bhn = containerEl.querySelector('.bhn-section');
              if (bhn && bhn.querySelector('.bhn-checkbox')?.checked) {
                const stbh = parseFormattedNumber(bhn.querySelector('.bhn-stbh')?.value || '0');
                const fee = calculateBhnPremium(personInfo, containerEl, startAge);
                const feeAdj = roundDownTo1000((Number(fee)||0) * freqFactor);
                const years = Math.max(0, Math.min(targetAge, 85) - startAge + 1);
                rows.push(`<tr>
                  <td class="p-2 border">${nameCell}</td>
              ...
              const acc = containerEl.querySelector('.accident-section');
              if (acc && acc.querySelector('.accident-checkbox')?.checked) {
                ...
                const fee = calculateAccidentPremium(personInfo, containerEl, startAge);
                const feeAdj = roundDownTo1000((Number(fee)||0) * freqFactor);
                const years = Math.max(0, Math.min(targetAge, 65) - startAge + 1);
                rows.push(`<tr>
                  <td class="p-2 border">${nameCell}</td>
                  <td class="p-2 border">Tai nạn</td>
                  <td class="p-2 border text-right">${formatCurrency(stbh)}</td>
                  <td class="p-2 border text-center">${years}</td>
                  <td class="p-2 border text-right">${feeAdj>0?formatCurrency(feeAdj):''}</td>
                </tr>`);
              ...
              const hs = containerEl.querySelector('.hospital-support-section');
              if (hs && hs.querySelector('.hospital-support-checkbox')?.checked) {
                ...
                const fee = calculateHospitalSupportPremium(personInfo, baseAnnual, containerEl, 0, startAge);
                const feeAdj = roundDownTo1000((Number(fee)||0) * freqFactor);
                const years = Math.max(0, Math.min(targetAge, 65) - startAge + 1);
                rows.push(`<tr>
                  <td class="p-2 border">${nameCell}</td>
              ...
              if (includeMdp3 && mdp3FeeAnnual > 0) {
                let stbhBase = 0;
                const pf = window.personFees || {};
                for (let pid in pf) stbhBase += (pf[pid].mainBase || 0) + (pf[pid].supp || 0);
                const feeAdj = roundDownTo1000((Number(mdp3FeeAnnual)||0) * freqFactor);
                const years = Math.max(0, Math.min(targetAge, 65) - startAge + 1);
                rows.push(`<tr>
                  <td class="p-2 border">${nameCell}</td>
                  ...
                  <td class="p-2 border text-center">${years}</td>
                  <td class="p-2 border text-right">${formatCurrency(feeAdj)}</td>
                </tr>`);
              }
              return rows.join('');
            }

            let mdp3SelectedId = null, mdp3FeeAnnual = 0;
            try{
              if(window.MDP3){
                mdp3SelectedId = (window.MDP3.getSelectedId && window.MDP3.getSelectedId()) || (document.getElementById('mdp3-person-select')?.value||null);
                mdp3FeeAnnual = Number(window.MDP3.getPremium ? (window.MDP3.getPremium()||0) : 0);
              }
            }catch(e){}

            let summaryPart1Html = `<div class="mb-4">
              <div class="text-lg font-semibold mb-1">Phần 1 · Tóm tắt sản phẩm</div>
              <div class="text-xs italic text-gray-500 mb-2">Các khoản phí dưới đây đã tính theo kỳ đóng phí đang chọn (Năm/Nửa năm/Quý). Phí bổ sung đã áp dụng hệ số kỳ.</div>
            ...
            // MAIN header row
            const mainSuppContainer = document.querySelector('#main-supp-container .supplementary-products-container');
            let mainSuppAnnual = 0;
            if (mainSuppContainer){
              mainSuppAnnual += calculateHealthSclPremium(mainPersonInfo, mainSuppContainer, mainPersonInfo.age);
              mainSuppAnnual += calculateBhnPremium(mainPersonInfo, mainSuppContainer, mainPersonInfo.age);
              mainSuppAnnual += calculateAccidentPremium(mainPersonInfo, mainSuppContainer, mainPersonInfo.age);
              mainSuppAnnual += calculateHospitalSupportPremium(mainPersonInfo, baseAnnual, mainSuppContainer, 0, mainPersonInfo.age);
            }
            const mainSuppAdj = roundDownTo1000(mainSuppAnnual * suppFactor);
            const mainMdp3 = (mdp3SelectedId === 'main-person-container') ? roundDownTo1000(mdp3FeeAnnual * suppFactor) : 0;
            const mainHeaderTotal = baseAnnual + extraAnnual + mainSuppAdj + mainMdp3;
            summaryPart1Html += `<tr class="bg-gray-50">
              <td class="p-2 border font-semibold">${sanitizeHtml(mainPersonInfo.name || 'NĐBH chính')}</td>
            ...
            const mainLabelMap = {
              ...
              'PUL_TRON_DOI':'PUL Trọn đời','PUL_15_NAM':'PUL 15 năm','PUL_5_NAM':'PUL 5 năm',
              'KHOE_BINH_AN':'Khỏe Bình An','VUNG_TUONG_LAI':'Vững Tương Lai'
            };
            const mainLabel = mainLabelMap[mainPersonInfo.mainProduct] || mainPersonInfo.mainProduct;
            const mainStbh = mainPersonInfo.mainProduct==='TRON_TAM_AN' ? 100000000 : parseFormattedNumber(document.getElementById('main-stbh')?.value || '0');
            const mainTerm = (mainPersonInfo.mainProduct==='AN_BINH_UU_VIET') ? (parseInt(document.getElementById('abuv-term')?.value,10)||'—') :
                              (mainPersonInfo.mainProduct==='TRON_TAM_AN' ? 10 : parseInt(document.getElementById('payment-term')?.value,10)||'—');
            summaryPart1Html += `<tr>
              <td class="p-2 border"></td>
              <td class="p-2 border">${sanitizeHtml(mainLabel)}</td>
              <td class="p-2 border text-right">${mainPersonInfo.mainProduct==='TRON_TAM_AN' ? '100.000.000' : formatCurrency(mainStbh)}</td>
            ...
            summaryPart1Html += buildSuppRows(mainPersonInfo, mainSuppContainer, { suppressName:true, freqFactor:suppFactor, includeMdp3:(mdp3SelectedId==='main-person-container'), mdp3FeeAnnual });
            ...
            document.querySelectorAll('#supplementary-insured-container .person-container').forEach((pCont)=>{
              ...
              const cont = pCont.querySelector('.supplementary-products-container');
              let suppAnnual = 0;
              if (cont){
                suppAnnual += calculateHealthSclPremium(pInfo, cont, pInfo.age);
                suppAnnual += calculateBhnPremium(pInfo, cont, pInfo.age);
                suppAnnual += calculateAccidentPremium(pInfo, cont, pInfo.age);
                suppAnnual += calculateHospitalSupportPremium(pInfo, baseAnnual, cont, 0, pInfo.age);
              }
              const suppAdj = roundDownTo1000(suppAnnual * suppFactor);
              const mdp3 = (mdp3SelectedId === pCont.id) ? roundDownTo1000(mdp3FeeAnnual * suppFactor) : 0;
              const headerTotal = suppAdj + mdp3;
              summaryPart1Html += `<tr class="bg-gray-50">
                <td class="p-2 border font-semibold">${sanitizeHtml(pInfo.name || 'NĐBH bổ sung')}</td>
                <td class="p-2 border">—</td>
                <td class="p-2 border text-right">—</td>
                <td class="p-2 border text-center">—</td>
                <td class="p-2 border text-right font-bold">${formatCurrency(headerTotal)}</td>
              </tr>`;

              summaryPart1Html += buildSuppRows(pInfo, cont, { suppressName:true, freqFactor:suppFactor, includeMdp3:(mdp3SelectedId===pCont.id), mdp3FeeAnnual });
            });

            if (mdp3SelectedId === 'other' && mdp3FeeAnnual > 0) {
              let stbhBase = 0;
              const pf = window.personFees || {};
              for (let pid in pf) stbhBase += (pf[pid].mainBase || 0) + (pf[pid].supp || 0);
              summaryPart1Html += `<tr>
                ...
                <td class="p-2 border text-right">${formatCurrency(stbhBase)}</td>
                <td class="p-2 border text-center">—</td>
                <td class="p-2 border text-right">${formatCurrency(roundDownTo1000(mdp3FeeAnnual * suppFactor))}</td>
              </tr>`;
            }

            summaryPart1Html += `</tbody></table></div>`;
            window._summaryPart1Html = summaryPart1Html;
          } catch(err){ window._summaryPart1Html = ''; }
        })();
        const summaryPart1Html = window._summaryPart1Html || '';
        ...
        container.innerHTML = (summaryPart1Html||'') + tableHtml;
...
function validateSection2FieldsPreCalc(customer) {
    ...
    if (mainProduct && mainProduct !== 'TRON_TAM_AN') {
        ...
        if (stbhEl) {
            ...
            if (!stbhEl.value || stbh <= 0) {
                setFieldError(stbhEl, 'Bắt buộc nhập STBH');
            } else if (stbh < 100000000) {
  ...
  window.renderSection6 = function renderSection6(){
    try{
      ...
      if(freqInfo.periods === 1){
        ...
        freqHtml = ``;
        let _rows = '';
_rows += `<div>Phí sản phẩm chính:</div><div class="text-right">${formatCurrency(freqInfo.breakdown.perMain)}</div>`;
if (freqInfo.breakdown.perExtra > 0) { _rows += `<div>Phí đóng thêm:</div><div class="text-right">${formatCurrency(freqInfo.breakdown.perExtra)}</div>`; }
if (freqInfo.breakdown.perSupp > 0) { _rows += `<div>Phí sản phẩm bổ sung:</div><div class="text-right">${formatCurrency(freqInfo.breakdown.perSupp)}</div>`; }
_rows += `<div class=\"font-semibold\">Tổng:</div><div class=\"font-semibold text-right\">${formatCurrency(freqInfo.perPeriod)}</div>`;
_rows += `<div>Tổng năm:</div><div class=\"text-right\">${formatCurrency(freqInfo.totalYearFromPeriod)}</div>`;
_rows += `<div>Chênh lệch:</div><div class=\"text-right ${freqInfo.diff<0?'text-green-600':(freqInfo.diff>0?'text-red-600':'')}\">${formatCurrency(freqInfo.diff)}</div>`;
freqHtml += `<div class=\"grid grid-cols-2 gap-2\">${_rows}</div>`;
      }
...
(function(){
  ...
  function fmt(v){ const n=Number(v)||0; return n>0 ? formatCurrency(n) : ''; }
...
if (suppListEl) {
  ...
  const personFees = window.personFees || {};
  const mainSupp = Number((personFees['main-person-container']?.supp) || 0);
  ...
  suppListEl.appendChild(mainRow);

  // For each supplementary person
  document.querySelectorAll('#supplementary-insured-container .person-container').forEach((cont, idx) => {
    ...
    const suppFee = Number((personFees[cont.id]?.supp) || 0) + ((mdp3SelectedId === cont.id) ? mdp3Fee : 0);
    const row = document.createElement('div');
    row.className = 'flex justify-between items-center py-1 text-sm';
    row.innerHTML = `<span>${sanitizeHtml(name)}</span><span class="font-semibold">${formatCurrency(suppFee)}</span>`;
    suppListEl.appendChild(row);
  });

  // If MDP3 is assigned to "other", add a separate line
...
(() => {
  ...
  function fmt(n){ const v=Number(n)||0; return v>0 ? formatCurrency(v) : ''; }
  function getFreq(){
    const sel = document.getElementById('payment-frequency');
    ...
    try{
      ...
      if(list){
        ...
        document.querySelectorAll('#supplementary-insured-container .person-container').forEach((cont, idx)=>{
          ...
          let displaySupp = Number(pf.supp||0);
          try{
            if(window.MDP3){
              const selId = (window.MDP3.getSelectedId ? window.MDP3.getSelectedId() : (document.getElementById('mdp3-person-select')?.value||null));
              const fee = Number(window.MDP3.getPremium ? (window.MDP3.getPremium()||0) : 0);
              if(selId === cont.id) displaySupp += fee;
            }
          }catch(e){}
          const row = document.createElement('div');
          row.className = 'flex justify-between items-center py-1 text-sm';
          row.innerHTML = `<span>Phí sản phẩm bổ sung của ${sanitizeHtml(name)}:</span><span class="font-semibold">${fmt(displaySupp)}</span>`;
      ...
      if(fb){
        if(freq==='year'){
          ...
          // Ẩn dòng nếu giá trị = 0
          try {
            if (el1) { const row1 = el1.closest("div"); if (row1) row1.classList.toggle("hidden", (info.perMain + info.perExtra) === 0); }
            if (el2) { const row2 = el2.closest("div"); if (row2) row2.classList.toggle("hidden", info.perSupp === 0); }
            // Tổng và Quy năm luôn hiện
            if (el5) { const row5 = el5.closest("div"); if (row5) row5.classList.toggle("hidden", info.diff === 0); }
          } catch(e){}
        }
      }
    }catch(err){
  ...
  window.generateSummaryTable = function(){
    ...
    try{
      ...
      const showCell = (v) => v>0 ? formatCurrency(v) : '';for(let i=0; (mainInfo.age + i) <= targetAge; i++){
        const yr = i + 1;
        ...
        html += `<td class="p-2 border text-right">${showCell(mainThisYear)}</td>`;
        html += `<td class="p-2 border text-right">${showCell(extraThisYear)}</td>`;
        html += `<td class="p-2 border text-right">${showCell(suppMain)}</td>`;
        suppEachArr.forEach(s => { html += `<td class="p-2 border text-right">${showCell(s)}</td>`; });
        html += `<td class="p-2 border text-right">${formatCurrency(totalFromPeriod)}</td>`;
        html += `<td class="p-2 border text-right">${periods===1 ? '' : formatCurrency(diff)}</td>`;
        html += `</tr>`;
      }
