...
// Gán sự kiện cho nút "Xem từng người" (giữ nguyên text)
function setupSuppToggle() {
...
// Prepare MDP3 selection to attribute fee to the correct person
let _mdp3SelId = null, _mdp3Fee = 0;
try {
  if (window.MDP3 && document.getElementById('mdp3-enable')?.checked) {
    _mdp3SelId = window.MDP3.getSelectedId && window.MDP3.getSelectedId();
    _mdp3Fee = Number(window.MDP3.getPremium ? (window.MDP3.getPremium()||0) : 0);
  }
} catch {}

  // NĐBH chính – phí bổ sung
  let mainSupp = (window.personFees?.['main-person-container']?.supp) || 0;
  if (_mdp3SelId==='main-person-container') mainSupp += _mdp3Fee;
...
// Helper: lấy thông tin NĐBH chính (tên, tuổi, giới tính) từ UI
function getMainPersonInfo() {
  const cont = document.getElementById('main-person-container');
  if (!cont) return {name: 'NĐBH chính', age: 0, gender: 'Nam'};
  const name = (cont.querySelector('.name-input')?.value || '').trim() || 'NĐBH chính';
  const age = Number(cont.querySelector('.age-span')?.textContent || '0') || 0;
  const gender = cont.querySelector('.gender-select')?.value || 'Nam';
...
function generateSummaryTable() {
  ...
  const getFreq = () => (document.getElementById('payment-frequency')?.value || 'year');
  const periodsMap = { year: 1, half: 2, quarter: 4 };
  const factorMap = { year: 1, half: 1.02, quarter: 1.04 };
  const freq = getFreq();
  const periods = periodsMap[freq] || 1;
  const factor = factorMap[freq] || 1;
  const freqLabel = freq === 'year' ? 'năm' : (freq === 'half' ? 'nửa năm' : 'quý');

  try {
    // read base annual totals from the compact card (already computed correctly)
    const mainAnnual = parseFormattedNumber(document.getElementById('summary-main-fee')?.textContent || '0');
    const extraAnnual = parseFormattedNumber(document.getElementById('summary-extra-fee')?.textContent || '0');
    let suppAnnualAll = parseFormattedNumber(document.getElementById('summary-supp-fee')?.textContent || '0');

    // Re-check MDP3: if selected to a specific supplementary person, ensure it's counted for that person, not lost
    let mdp3SelectedId = null, mdp3Annual = 0;
    try {
      if (window.MDP3 && document.getElementById('mdp3-enable')?.checked) {
        mdp3SelectedId = window.MDP3.getSelectedId && window.MDP3.getSelectedId();
        mdp3Annual = Number(window.MDP3.getPremium()||0);
      }
    } catch(e){}

    // period conversion
    const toPeriod = (annual) => {
      if (periods === 1) return Math.round(annual);
      return Math.round((Number(annual||0) / periods) * factor);
    };

    // context info
    const mainPersonInfo = getMainPersonInfo();
    const targetAge = Number(document.getElementById('target-age-input')?.value || mainPersonInfo.age || 0) || mainPersonInfo.age || 0;
    const mainProduct = document.getElementById('main-product')?.value || '';
    const productLabel = getProductLabel(mainProduct);
    const mainStbh = getMainStbh();
    const mainTerm = getPaymentTerm();

    // build supplementary persons array
    const suppPersons = Array.from(document.querySelectorAll('#supplementary-insured-container .person-container')).map(cont => ({
      id: cont.id,
      container: cont,
      name: cont.querySelector('.name-input')?.value || 'NĐBH bổ sung',
      age: Number(cont.querySelector('.age-span')?.textContent || '0') || 0,
      gender: cont.querySelector('.gender-select')?.value || 'Nam',
      riskGroup: cont.querySelector('.risk-group-span')?.textContent || '—'
    }));

    // per-person supplementary annual totals from cached computation (window.personFees)
    const personFees = (window.personFees || {});

    // ===== PHẦN 1: TÓM TẮT (theo kỳ người dùng chọn) =====
    let summaryHtml = `<div class="mb-4">
      <div class="text-lg font-semibold mb-2">Phần 1 · Tóm tắt sản phẩm (phí theo ${freqLabel})</div>
      <table class="w-full text-left border-collapse">
        <thead class="bg-gray-100">
          <tr>
            ...
            <th class="p-2 border">Phí đóng (${freqLabel})</th>
          </tr>
        </thead>
        <tbody>`;

    // helper to render a person header row with total in the right-most cell (bold)
    const personHeaderRow = (personName, totalPeriod) => {
      const safe = sanitizeHtml(personName || '—');
      return `<tr>
        <td class="p-2 border font-semibold" colspan="3">${safe}</td>
        <td class="p-2 border text-right font-bold">${formatCurrency(totalPeriod)}</td>
      </tr>`;
    };

    // MAIN person block
    const mainPeriod = toPeriod(mainAnnual + extraAnnual + (Number(personFees['main-person-container']?.supp||0)));
    summaryHtml += personHeaderRow(mainPersonInfo.name || 'NĐBH chính', mainPeriod);
    ...
    if (mainProduct) {
      summaryHtml += `<tr>
        ...
        <td class="p-2 border text-right">${formatCurrency(toPeriod(mainAnnual))}</td>
      </tr>`;
    }

    // Extra premium row (hide if zero)
    if (Number(extraAnnual) > 0) {
      summaryHtml += `<tr>
        <td class="p-2 border">Phí đóng thêm</td>
        <td class="p-2 border text-right">—</td>
        <td class="p-2 border text-center">${mainTerm || '—'}</td>
        <td class="p-2 border text-right">${formatCurrency(toPeriod(extraAnnual))}</td>
      </tr>`;
    }

    // Supplementary rows for MAIN person
    summaryHtml += buildSupplementSummaryRows_PerPersonOnly(mainPersonInfo, document.querySelector('#main-supp-container .supplementary-products-container'), targetAge, toPeriod);

    // MDP3: if selected to MAIN, show it under main with STBH and period premium
    if (mdp3SelectedId === 'main-person-container' && mdp3Annual > 0) {
      const stbhBase = (window.MDP3 && window.MDP3.getStbhBase && window.MDP3.getStbhBase()) || (window.MDP3?.getStbh && window.MDP3.getStbh()) || 0;
      const mdpYears = Math.max(0, Math.min(targetAge, 65) - (mainPersonInfo.age||0) + 1);
      summaryHtml += `<tr>
        <td class="p-2 border">Miễn Đóng Phí 3.0</td>
        <td class="p-2 border text-right">${formatCurrency(stbhBase||0)}</td>
        <td class="p-2 border text-center">${mdpYears}</td>
        <td class="p-2 border text-right">${formatCurrency(toPeriod(mdp3Annual))}</td>
      </tr>`;
    }

    // SUPPLEMENTARY persons blocks
    suppPersons.forEach(p => {
      const baseSuppAnnual = Number(personFees[p.id]?.supp || 0);
      const includeMdp3 = (mdp3SelectedId === p.id) ? mdp3Annual : 0;
      const totalPer = toPeriod(baseSuppAnnual + includeMdp3);
      summaryHtml += personHeaderRow(p.name || 'NĐBH bổ sung', totalPer);

      // rows for that person's supplementary products
      const cont = p.container.querySelector('.supplementary-products-container');
      summaryHtml += buildSupplementSummaryRows_PerPersonOnly(p, cont, targetAge, toPeriod);

      // MDP3 selected to this supplementary person
      if (includeMdp3 > 0) {
        const stbhBase = (window.MDP3 && window.MDP3.getStbhBase && window.MDP3.getStbhBase()) || (window.MDP3?.getStbh && window.MDP3.getStbh()) || 0;
        const mdpYears = Math.max(0, Math.min(targetAge, 65) - (p.age||0) + 1);
        summaryHtml += `<tr>
          <td class="p-2 border">Miễn Đóng Phí 3.0</td>
          <td class="p-2 border text-right">${formatCurrency(stbhBase||0)}</td>
          <td class="p-2 border text-center">${mdpYears}</td>
          <td class="p-2 border text-right">${formatCurrency(toPeriod(includeMdp3))}</td>
        </tr>`;
      }
    });

    // If MDP3 is "Người khác", show a small line for completeness (not attached to a person block)
    if (mdp3SelectedId === 'other' && mdp3Annual > 0) {
      const stbhBase = (window.MDP3 && window.MDP3.getStbhBase && window.MDP3.getStbhBase()) || (window.MDP3?.getStbh && window.MDP3.getStbh()) || 0;
      const mdpYears = '—';
      summaryHtml += `<tr>
        <td class="p-2 border">Miễn Đóng Phí 3.0 (Người khác)</td>
        <td class="p-2 border text-right">${formatCurrency(stbhBase||0)}</td>
        <td class="p-2 border text-center">${mdpYears}</td>
        <td class="p-2 border text-right">${formatCurrency(toPeriod(mdp3Annual))}</td>
      </tr>`;
    ...
    // ===== PHẦN 2: BẢNG PHÍ (theo ${freqLabel}) =====
    const headerCols = [
      '<th class="p-2 border text-center">Năm HĐ</th>',
      '<th class="p-2 border text-center">Tuổi (NĐBH chính)</th>',
      '<th class="p-2 border text-right">Phí chính</th>',
      '<th class="p-2 border text-right">Đóng thêm</th>',
      '<th class="p-2 border text-right">Bổ sung</th>'
    ];
    // Add dynamic supplemental columns per product if available
    const suppNames = getSupplementNamesForMain(); // returns array of label strings (stable across years)
    const suppThs = (suppNames||[]).map(() => '<th class="p-2 border text-right">SP bổ sung</th>');

    // Chênh lệch column only when freq != 'year'
    const showDiff = (periods > 1);

    let tableHtml = `<div class="mt-6">
      <div class="text-lg font-semibold mb-2">Phần 2 · Bảng Phí (theo ${freqLabel})</div>
      <table class="w-full text-left border-collapse">
        <thead class="bg-gray-100">
          <tr>${headerCols.join('')}${suppThs.join('')}<th class="p-2 border text-right font-semibold">Tổng (${freqLabel})</th>${showDiff?'<th class="p-2 border text-right">Chênh lệch so với năm</th>':''}</tr>
        </thead>
        <tbody>`;

    // compute for each contract year using existing logic from current function (mainThisYearBase + extraThisYear + supp premiums etc.)
    const maxYear = Math.max(1, Math.min(100, targetAge - (mainPersonInfo.age||0) + 1));
    for (let i=1; i<=maxYear; i++) {
      const contractYear = i;
      const currentAgeMain = (mainPersonInfo.age||0) + i - 1;

      // reuse existing per-year functions (present elsewhere in file)
      const mainThisYearBase = calculateMainProductAnnualAtYear(contractYear) || 0;
      const extraThisYear = calculateExtraPremiumAnnualAtYear(contractYear) || 0;
      const suppPremiumMain = calculateSuppPremiumAnnualAtYearForMain(contractYear) || 0;
      const suppPremiums = calculateSuppPremiumsAnnualAtYearPerProduct(contractYear) || [];

      const perMainExtra = toPeriod(mainThisYearBase + extraThisYear);
      const perSupp = toPeriod(suppPremiumMain + suppPremiums.reduce((a,b)=>a+Number(b||0),0));
      const totalPeriod = perMainExtra + perSupp;

      // yearly equivalent reconstructed from period choice (for diff)
      const yearFromPeriod = (periods === 1) ? (mainThisYearBase + extraThisYear + suppPremiumMain + suppPremiums.reduce((a,b)=>a+Number(b||0),0))
                                            : totalPeriod * periods;
      const diff = yearFromPeriod - (mainThisYearBase + extraThisYear + suppPremiumMain + suppPremiums.reduce((a,b)=>a+Number(b||0),0));
      ...
      tableHtml += `<tr>
        ...
        <td class="p-2 border text-right">${formatCurrency(extraThisYear || 0)}</td>
        <td class="p-2 border text-right">${formatCurrency(suppPremiumMain)}</td>`;
      suppPremiums.forEach(sp => { tableHtml += `<td class="p-2 border text-right">${formatCurrency(sp)}</td>`; });
      tableHtml += `<td class="p-2 border text-right font-semibold">${formatCurrency(totalPeriod)}</td>`;
      if (showDiff) { tableHtml += `<td class="p-2 border text-right">${formatCurrency(diff)}</td>`; }
      tableHtml += `</tr>`;
    }
...
// Helpers used by the new summary table (thin wrappers reusing existing logic/data)
function buildSupplementSummaryRows_PerPersonOnly(personInfo, container, targetAge, toPeriodFn) {
  if (!container) return '';
  const rows = [];
  ...
  if (sclSec && sclSec.querySelector('.health-scl-checkbox')?.checked) {
    ...
    rows.push(`<tr>
      ...
      <td class="p-2 border text-right">${formatCurrency(toPeriodFn(feeAnnual))}</td>
    </tr>`);
  }
...
// lightweight helpers that existing code is expected to have
function getMainStbh() {
  const val = document.querySelector('#main-product-options .stbh-input')?.value || '0';
  return parseFormattedNumber(val);
}
function getPaymentTerm() {
  const el = document.querySelector('#main-product-options .payment-term-select');
  return el ? el.value : '';
}
function getSupplementNamesForMain(){
  const cont = document.querySelector('#main-supp-container .supplementary-products-container');
  if (!cont) return [];
  const names = [];
  if (cont.querySelector('.health-scl-checkbox')?.checked) names.push('SCL');
  if (cont.querySelector('.bhn-checkbox')?.checked) names.push('BHN');
  if (cont.querySelector('.accident-checkbox')?.checked) names.push('TN');
  return names;
}

// Stubs expected to exist; if not, define safe fallbacks
if (typeof calculateMainProductAnnualAtYear !== 'function') {
  window.calculateMainProductAnnualAtYear = function(y){ return 0; };
}
if (typeof calculateExtraPremiumAnnualAtYear !== 'function') {
  window.calculateExtraPremiumAnnualAtYear = function(y){ return 0; };
}
if (typeof calculateSuppPremiumAnnualAtYearForMain !== 'function') {
  window.calculateSuppPremiumAnnualAtYearForMain = function(y){ return 0; };
}
if (typeof calculateSuppPremiumsAnnualAtYearPerProduct !== 'function') {
  window.calculateSuppPremiumsAnnualAtYearPerProduct = function(y){ return []; };
}
...
(function(){
  ...
  function render(){
    ...
    // Hide zero-value rows under frequency breakdown
try{
  const rowSupp = document.getElementById('freq-supp-total')?.parentElement;
  if(rowSupp){ rowSupp.classList.toggle('hidden', perSupp===0); }
  const rowDiff = document.getElementById('freq-diff')?.parentElement;
  if(rowDiff){ rowDiff.classList.toggle('hidden', diff===0); }
}catch(e){}
...
(function() {
  ...
  function updateBadge() { /* disabled per user request */ }
}
)();
