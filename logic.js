...
function initPerson(container, personId, isSupp = false) {
  ...
  initOccupationAutocomplete(container.querySelector('.occupation-input'), container);
    ...
    // For main person inputs, run validation and recalculate on change so age and premiums update promptly
    if (nameInput) {
      nameInput.addEventListener('input', () => {
        validateMainPersonInputs();
        // recalc on name input to update premium breakdown if needed
    ...
    if (dobInput) {
      dobInput.addEventListener('input', () => {
        validateMainPersonInputs();
        if (window.MDP3) MDP3.resetIfEnabled();
        try { calculateAll(); } catch (e) {}
      });
...
function calculateAll() {
  try {
    ...
    const mainPersonContainer = document.getElementById('main-person-container');
    const mainPersonInfo = getCustomerInfo(mainPersonContainer, true);
    // Always reflect age of main person immediately on recalculation
    try {
      const ageSpanEl = mainPersonContainer.querySelector('.age-span');
      if (ageSpanEl) ageSpanEl.textContent = mainPersonInfo.age;
    } catch (e) {}
