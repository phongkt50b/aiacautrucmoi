 ...
 * - Hoạt động song song với logic cũ (kể cả type="module")
 * - Fix: nhãn nút "Xem từng người" giữ nguyên; cộng MDP3 đúng người
 * - Dọn khối "Kỳ đóng phí": xóa "Kỳ: …", ẩn dòng = 0, ẩn toàn khối khi chọn Năm
 * - Báo cáo (Phần 1 & 2) theo đặc tả của bạn; ẩn ô = 0; cột Chênh lệch chỉ hiện khi nửa năm/quý
 * ========================= */
(function(){
  ...
  function generateSummaryTablePatched(){
    ...
    try{
      ...
      // Bổ sung của NĐBH chính (kể cả MDP3 nếu chọn vào chính)
      ...
      qa('#supplementary-insured-container .person-container').forEach(cont => {
        ...
        // Các dòng sản phẩm bổ sung của người đó
        html1 += suppRows(p, suppCont, mdp.id===cont.id);
      });

      // Trường hợp MDP3 chọn "Người khác"
      ...
      // === PHẦN 2: Bảng phí ===
      const suppPersons2 = qa('.person-container')
        .filter(x=>x.id!=='main-person-container')
        .map(x => window.getCustomerInfo?window.getCustomerInfo(x,false):null)
        .filter(Boolean);
      ...
      for (let i=0; mainInfo.age+i<=targetAge; i++){
        ...
        // Bổ sung theo tuổi chạy năm i (NĐBH chính)
        let suppMainY = 0;
        ...
        // Thêm MDP3 theo đối tượng đã chọn
        if (mdp.fee>0 && mdp.id){
        ...
        // Quy đổi theo kỳ (bổ sung áp dụng hệ số)
  ...
  // ===== 7) Xuất ra window để nút báo cáo gọi được (kể cả khi type="module") =====
  try {
