...
/* =========================
 * HOTFIX – UI & TÍNH PHÍ (KHÔNG ĐỤNG LOGIC GỐC)
 * Dán nguyên khối này vào CUỐI file logic.js hiện tại của bạn
 * ========================= */

(function(){
  // ===== Helpers an toàn (chỉ khai báo nếu chưa có) =====
  window.sanitizeHtml = window.sanitizeHtml || function(str=''){
    return String(str)
      .replace(/&/g,'&amp;')
      .replace(/</g,'&lt;')
      .replace(/>/g,'&gt;')
  ...
  window.formatCurrency = window.formatCurrency || function(n){
    return (Number(n)||0).toLocaleString('vi-VN'); // KHÔNG thêm "VND"
  };

  window.roundDownTo1000 = window.roundDownTo1000 || function(n){
    return Math.floor((Number(n)||0)/1000)*1000;
  };

  window.parseFormattedNumber = window.parseFormattedNumber || function(v){
  ...
  // ===== 1) Render danh sách “Xem từng người” – cố định nội dung & cộng MDP3 đúng người =====
  window.renderSuppListSimple = function renderSuppListSimple(){
    const wrap = $('#supp-insured-summaries');
    if (!wrap) return;
    wrap.innerHTML = '';

    // MDP3: đọc người được gán & phí
    let mdp3SelId = null, mdp3Fee = 0;
    try {
    ...
    const mainSum  = mainSupp + ((mdp3SelId === mainId) ? mdp3Fee : 0);
    const rMain    = document.createElement('div');
    rMain.className = 'flex justify-between items-center py-1 text-sm';
    rMain.innerHTML = `<span>${sanitizeHtml(mainName)}</span><span class="font-semibold">${formatCurrency(mainSum)}</span>`;
    wrap.appendChild(rMain);

    // NĐBH bổ sung
    $$('#supplementary-insured-container .person-container').forEach((cont, i) => {
      const id   = cont.id;
      const name = cont.querySelector('.name-input')?.value?.trim() || `NĐBH bổ sung ${i+1}`;
      const fee  = (window.personFees?.[id]?.supp) || 0;
      const sum  = fee + ((mdp3SelId === id) ? mdp3Fee : 0);
      const row  = document.createElement('div');
      row.className = 'flex justify-between items-center py-1 text-sm';
      row.innerHTML = `<span>${sanitizeHtml(name)}</span><span class="font-semibold">${formatCurrency(sum)}</span>`;
      wrap.appendChild(row);
    });
    // Theo yêu cầu: KHÔNG hiển thị dòng “Kỳ: …”/tiền tố “Phí …” và KHÔNG thêm dòng riêng cho MDP3 “Người khác”.
  };

  // Luôn khôi phục danh sách ngay sau bất kỳ re-render nào khác
  const reapplySuppList = () => { try { window.renderSuppListSimple(); } catch(e){} };

  // ===== 2) Giữ nhãn nút “Xem từng người” không đổi & chỉ toggle danh sách =====
  (function freezeToggleBtn(){
    const btn  = $('#toggle-supp-list-btn');
    const list = $('#supp-insured-summaries');
    if (!btn || !list) return;

    const keepLabel = () => { btn.textContent = 'Xem từng người'; };
    keepLabel();

    // Ưu tiên handler bắt sớm (capture) để chặn thay đổi nhãn từ các handler khác
  ...
  function cleanupFrequencyPanel(){
    ...
    if (first && /^Kỳ\s*:/.test(first.textContent.trim())) {
      first.remove();
    }

    const hideIfZeroById = (id) => {
      const el = $('#'+id);
      if (!el) return;
      const n = parseFormattedNumber(el.textContent||'0');
      const row = el.closest('div');
      if (row) row.classList.toggle('hidden', n === 0);
    };
    hideIfZeroById('freq-main-plus-extra');
    hideIfZeroById('freq-supp-total');
    hideIfZeroById('freq-total-period');
    hideIfZeroById('freq-total-year');
    hideIfZeroById('freq-diff');

    // Hiện panel nếu có kỳ khác năm; ẩn nếu chọn Năm
    const freq = $('#payment-frequency')?.value || 'year';
    pane.classList.toggle('hidden', freq === 'year');
  }

  // Gắn vào change của kỳ đóng phí
  $('#payment-frequency')?.addEventListener('change', () => {
    setTimeout(() => { reapplySuppList(); cleanupFrequencyPanel(); }, 0);
  });

  // ===== 4) Bỏ thêm chữ “VND” ở nơi khác (nếu có hàm fmt cũ) =====
  // (Không chạm logic tính; chỉ định nghĩa lại fmt nếu có)
  if (typeof window.fmt === 'function') {
    try {
      window.fmt = (n) => window.formatCurrency(Number(n)||0);
    } catch(e){}
  }

  // ===== 5) Gói lại render tổng phía phải: ẩn dòng bằng 0 =====
  (function wrapUpdateSummaryUI(){
    const origUpdate = window.updateSummaryUI;
    window.updateSummaryUI = function(){
      try { origUpdate && origUpdate.apply(this, arguments); } catch(e){}
      // Ẩn dòng = 0
      const hideIfZero = (id) => {
        const el = $('#'+id);
        if (!el) return;
      ...
      hideIfZero('main-insured-extra-fee');
      hideIfZero('summary-supp-fee');
      cleanupFrequencyPanel();
      reapplySuppList();
    };
  })();

  // ===== 6) Ghi đè generateSummaryTable theo đặc tả mới =====
  window.generateSummaryTable = function generateSummaryTable(){
    const modal = $('#summary-modal');
    const container = $('#summary-content-container');
    container.innerHTML = '';

    // Helpers hàm gốc dự kiến đã có
    const getInfo = window.getCustomerInfo;
    const calcMain = window.calculateMainPremium;
    const extraVal = window.getExtraPremiumValue;
    const calcSCL  = window.calculateHealthSclPremium;
    const calcBHN  = window.calculateBhnPremium;
    const calcACC  = window.calculateAccidentPremium;
    const calcHS   = window.calculateHospitalSupportPremium;
    const getLabel = window.getProductLabel;

    const rd1000 = window.roundDownTo1000;

    try {
      const tAgeEl = $('#target-age-input');
      const tAge   = parseInt(tAgeEl?.value||'0',10);
      const mainC  = $('#main-person-container');
      const main   = getInfo(mainC, true);
      const product= main.mainProduct;

      if (!tAge || tAge <= main.age) throw new Error('Vui lòng nhập độ tuổi mục tiêu hợp lệ (> tuổi hiện tại).');

      // Ràng buộc của TTA
      if (product === 'TRON_TAM_AN') {
        const mainSuppC = $('#main-supp-container .supplementary-products-container');
        const sclSec  = mainSuppC?.querySelector('.health-scl-section');
        const sclChk  = sclSec?.querySelector('.health-scl-checkbox');
        const sclFee  = calcSCL(main, mainSuppC, main.age);
        if (!sclChk?.checked || !sclFee) throw new Error('Trọn Tâm An bắt buộc kèm Sức Khỏe Bùng Gia Lực với phí hợp lệ.');
      }

      // Số năm đóng phí
      let payTerm = 0;
      if (product === 'TRON_TAM_AN') payTerm = 10;
      else if (product === 'AN_BINH_UU_VIET') payTerm = parseInt($('#abuv-term')?.value||'0',10);
      else payTerm = parseInt($('#payment-term')?.value||'0',10) || 0;

      if (['PUL_TRON_DOI','PUL_5_NAM','PUL_15_NAM','KHOE_BINH_AN','VUNG_TUONG_LAI'].includes(product)) {
        const minAge = main.age + (payTerm||0) - 1;
        if (tAge < minAge) throw new Error(`Độ tuổi mục tiêu phải ≥ ${minAge} với sản phẩm hiện tại.`);
      }

      // Người bổ sung
      const others = $$('.person-container').filter(x => x.id !== 'main-person-container').map(x => getInfo(x, false));

      // Phí chính/năm
      const baseMainAnnual = calcMain(main);
      const extraAnnual = extraVal();
      const mainAnnual  = baseMainAnnual + extraAnnual;

      // Kỳ
      const freq  = $('#payment-frequency')?.value || 'year';
      const per   = (freq==='half')?2:((freq==='quarter')?4:1);
      const suppK = (freq==='half')?1.02:((freq==='quarter')?1.04:1);

      // Product label & STBH/term
      const label = getLabel(product);
      const mainStbh = (product==='TRON_TAM_AN') ? 100_000_000 : parseFormattedNumber($('#main-stbh')?.value||'0');
      const mainTerm = (product==='TRON_TAM_AN') ? 10 : (product==='AN_BINH_UU_VIET' ? parseInt($('#abuv-term')?.value||'0',10) : parseInt($('#payment-term')?.value||'0',10));

      // MDP3
      let mdp3SelId = null, mdp3Annual = 0;
      if ($('#mdp3-enable')?.checked && window.MDP3) {
        mdp3SelId   = window.MDP3.getSelectedId && window.MDP3.getSelectedId();
        mdp3Annual  = Number(window.MDP3.getPremium ? (window.MDP3.getPremium()||0) : 0);
      }

      const show = (v)=> v>0 ? formatCurrency(v) : '';
      const toYr = (age, max) => Math.max(0, Math.min(tAge, max) - age + 1);

      // Tổng bổ sung theo người (đã áp dụng hệ số kỳ)
      function personSuppAnnual(person, cont){
        if (!cont) return 0;
        let s = 0;
        s += calcSCL(person, cont, person.age);
        s += calcBHN(person, cont, person.age);
        s += calcACC(person, cont, person.age);
        s += calcHS(person, baseMainAnnual, cont, 0, person.age);
        return rd1000(s * suppK);
      }

      // Rows bổ sung cho từng người
      function suppRows(person, cont, includeMdp3Here){
        let html = '';
        ...
        if (scl && scl.querySelector('.health-scl-checkbox')?.checked) {
          ...
          const fee  = rd1000(calcSCL(person, cont, person.age)*suppK);
          html += `<tr><td class="p-2 border"></td><td class="p-2 border">Sức khoẻ Bùng Gia Lực ${labelProgram?'- '+labelProgram:''}</td><td class="p-2 border text-right">${formatCurrency(stbh)}</td><td class="p-2 border text-center">${toYr(person.age,75)}</td><td class="p-2 border text-right">${show(fee)}</td></tr>`;
        }
        ...
        if (bhn && bhn.querySelector('.bhn-checkbox')?.checked) {
          ...
          const fee  = rd1000(calcBHN(person, cont, person.age)*suppK);
          html += `<tr><td class="p-2 border"></td><td class="p-2 border">Bệnh hiểm nghèo 2.0</td><td class="p-2 border text-right">${formatCurrency(stbh)}</td><td class="p-2 border text-center">${toYr(person.age,85)}</td><td class="p-2 border text-right">${show(fee)}</td></tr>`;
        }
        ...
        if (acc && acc.querySelector('.accident-checkbox')?.checked) {
          ...
          const fee  = rd1000(calcACC(person, cont, person.age)*suppK);
          html += `<tr><td class="p-2 border"></td><td class="p-2 border">Bảo hiểm Tai nạn</td><td class="p-2 border text-right">${formatCurrency(stbh)}</td><td class="p-2 border text-center">${toYr(person.age,65)}</td><td class="p-2 border text-right">${show(fee)}</td></tr>`;
        }
        ...
        if (hs && hs.querySelector('.hospital-support-checkbox')?.checked) {
          ...
          const fee  = rd1000(calcHS(person, baseMainAnnual, cont, 0, person.age)*suppK);
          html += `<tr><td class="p-2 border"></td><td class="p-2 border">Hỗ trợ chi phí nằm viện (đ/ngày)</td><td class="p-2 border text-right">${formatCurrency(stbh)}</td><td class="p-2 border text-center">${toYr(person.age,65)}</td><td class="p-2 border text-right">${show(fee)}</td></tr>`;
        }

        if (includeMdp3Here && mdp3Annual>0) {
          // STBH cơ sở cho MDP3 = tổng STBH phí chính + bổ sung (trừ phần bổ sung của chính người này)
          let stbhBase = 0;
          const pf = window.personFees || {};
          for (const pid in pf) stbhBase += (pf[pid].mainBase||0) + (pf[pid].supp||0);
          if (pf[person.container?.id]) stbhBase -= (pf[person.container.id].supp||0);
          const feeAdj = rd1000(mdp3Annual*suppK);
          html += `<tr><td class="p-2 border"></td><td class="p-2 border">Miễn đóng phí 3.0</td><td class="p-2 border text-right">${formatCurrency(stbhBase)}</td><td class="p-2 border text-center">${toYr(person.age,65)}</td><td class="p-2 border text-right">${formatCurrency(feeAdj)}</td></tr>`;
        }
        return html;
      }
      ...
      let html1 = `<div class="mb-4">
        ...
        <div class="text-xs italic text-gray-500 mb-2">Các khoản phí dưới đây đã tính theo kỳ đóng phí đang chọn (Năm/Nửa năm/Quý). Lưu ý: chỉ phí bổ sung áp dụng hệ số theo kỳ.</div>
        <table class="w-full text-left border-collapse">
          <thead class="bg-gray-100">
            <tr>
              <th class="p-2 border">Tên NĐBH</th>
      ...
      // NĐBH chính – header tổng
      const mainSuppC = $('#main-supp-container .supplementary-products-container');
      const mainSuppAdj = personSuppAnnual(main, mainSuppC);
      const mainHeaderTotal = mainAnnual + mainSuppAdj + ((mdp3SelId==='main-person-container') ? rd1000(mdp3Annual*suppK) : 0);
      html1 += `<tr class="bg-gray-50">
        <td class="p-2 border font-semibold">${sanitizeHtml(main.name||'NĐBH chính')}</td>
        <td class="p-2 border">—</td><td class="p-2 border text-right">—</td><td class="p-2 border text-center">—</td>
        <td class="p-2 border text-right font-bold">${formatCurrency(mainHeaderTotal)}</td>
      </tr>`;

      // Dòng sản phẩm chính
      html1 += `<tr>
        <td class="p-2 border"></td>
        <td class="p-2 border">${sanitizeHtml(label)}</td>
        <td class="p-2 border text-right">${product==='TRON_TAM_AN' ? '100.000.000' : formatCurrency(mainStbh)}</td>
        <td class="p-2 border text-center">${mainTerm || '—'}</td>
        <td class="p-2 border text-right">${show(baseMainAnnual)}</td>
      </tr>`;

      // Phí đóng thêm (ẩn nếu 0)
      if (extraAnnual>0) {
        html1 += `<tr>
          <td class="p-2 border"></td>
          <td class="p-2 border">Phí đóng thêm</td>
      ...
      // Bổ sung NĐBH chính
      html1 += suppRows(main, mainSuppC, mdp3SelId==='main-person-container');

      // Từng NĐBH bổ sung
      others.forEach(p => {
        const cont = p.container.querySelector('.supplementary-products-container');
        const pSuppAdj = personSuppAnnual(p, cont);
        const pHeaderTotal = pSuppAdj + ((mdp3SelId===p.container.id) ? rd1000(mdp3Annual*suppK) : 0);
        html1 += `<tr class="bg-gray-50">
          <td class="p-2 border font-semibold">${sanitizeHtml(p.name||'NĐBH bổ sung')}</td>
          <td class="p-2 border">—</td><td class="p-2 border text-right">—</td><td class="p-2 border text-center">—</td>
          <td class="p-2 border text-right font-bold">${formatCurrency(pHeaderTotal)}</td>
        </tr>`;
        html1 += suppRows(p, cont, mdp3SelId===p.container.id);
      });

      // MDP3 gán cho “Người khác”
      if (mdp3SelId==='other' && mdp3Annual>0) {
        let stbhBase = 0;
        const pf = window.personFees || {};
        for (const pid in pf) stbhBase += (pf[pid].mainBase||0) + (pf[pid].supp||0);
        html1 += `<tr>
          ...
          <td class="p-2 border text-right">${formatCurrency(rd1000(mdp3Annual*suppK))}</td>
        </tr>`;
      }
      ...
      html2 += `<table class="w-full text-left border-collapse"><thead class="bg-gray-100"><tr>
        ...
        <th class="p-2 border">Tuổi NĐBH chính<br>(${sanitizeHtml(main.name)})</th>
        <th class="p-2 border">Phí chính</th>
        <th class="p-2 border">Phí đóng thêm</th>
        <th class="p-2 border">Phí bổ sung<br>(${sanitizeHtml(main.name)})</th>`;

      others.forEach(p => {
        html2 += `<th class="p-2 border">Phí bổ sung<br>(${sanitizeHtml(p.name)})</th>`;
      });
      html2 += `<th class="p-2 border">Tổng cộng</th>`;
      if (per>1) html2 += `<th class="p-2 border">Chênh lệch so với năm</th>`;
      html2 += `</tr></thead><tbody>`;

      for (let i=0; main.age+i<=tAge; i++){
        const age = main.age + i;
        const year= i+1;

        const mainBaseY = (year<=payTerm) ? baseMainAnnual : 0;
        const extraY    = (year<=payTerm) ? extraAnnual : 0;

        let suppMainY = 0;
        const mCont = $('#main-supp-container .supplementary-products-container');
        if (mCont){
          suppMainY += calcSCL({...main, age}, mCont, age);
          suppMainY += calcBHN({...main, age}, mCont, age);
          suppMainY += calcACC({...main, age}, mCont, age);
          suppMainY += calcHS({...main, age}, baseMainAnnual, mCont, 0, age);
        }

        const suppEachY = others.map(p=>{
          let s=0;
          const cont = p.container.querySelector('.supplementary-products-container');
          const ageP = p.age + i;
          if (cont){
            s += calcSCL({...p, age:ageP}, cont, ageP);
            s += calcBHN({...p, age:ageP}, cont, ageP);
            s += calcACC({...p, age:ageP}, cont, ageP);
            s += calcHS({...p, age:ageP}, baseMainAnnual, cont, 0, ageP);
          }
          return s;
        });

        // Cộng MDP3 vào đúng cột người được gán
        if (mdp3Annual>0 && mdp3SelId){
          if (mdp3SelId==='main-person-container') suppMainY += mdp3Annual;
          else if (mdp3SelId!=='other'){
            const idx = others.findIndex(x => x.container?.id===mdp3SelId);
            if (idx>=0) suppEachY[idx] += mdp3Annual;
          }
        }

        const annualSupp = suppMainY + suppEachY.reduce((a,b)=>a+b,0);

        const perMainExtra = (per===1) ? (mainBaseY + extraY) : Math.floor((mainBaseY + extraY)/per);
        const perSupp      = (per===1) ? annualSupp : Math.round((annualSupp/1000*suppK/per))*1000;
        const perTotal     = perMainExtra + perSupp;
        const totalFromPer = perTotal*per;
        const diff         = totalFromPer - (mainBaseY + extraY + annualSupp);
        ...
        html2 += `<tr>
          ...
          <td class="p-2 border text-right">${cell(mainBaseY)}</td>
        ...
        html2 += `<td class="p-2 border text-right font-semibold">${formatCurrency(totalFromPer)}</td>`;
        if (per>1) html2 += `<td class="p-2 border text-right">${formatCurrency(diff)}</td>`;
        html2 += `</tr>`;
      }
      ...
      $('#export-html-btn')?.addEventListener('click', () => window.print());

    } catch(e){
      container.innerHTML = `<p class="text-red-600 font-semibold text-center">${sanitizeHtml(e.message||String(e))}</p>`;
    } finally {
      $('#summary-modal')?.classList.remove('hidden');
    }
  };

  // ===== 7) Chặn các hàm render khác ghi đè danh sách từng người sau khi đổi kỳ =====
  ['renderSection6','renderSection6V2'].forEach(fn => {
    const orig = window[fn];
    if (typeof orig === 'function') {
      window[fn] = function(){
        try { orig.apply(this, arguments); } catch(e){}
        // Sau khi các hàm cũ render xong -> phục hồi danh sách & dọn panel kỳ
  ...
  // Lần đầu vào trang: dọn panel kỳ & render danh sách
  setTimeout(() => { try{ window.renderSuppListSimple(); }catch(e){} cleanupFrequencyPanel(); }, 0);
})();
