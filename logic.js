...
function renderSuppListSimple() {
  ...
  const mainTotal = mainSupp + ((mdp3SelId === mainId) ? mdp3Fee : 0);
  const rowMain = document.createElement('div');
  rowMain.className = 'flex justify-between items-center py-1 text-sm';
  rowMain.innerHTML = `<span>${sanitizeHtml(mainName)}</span><span class="font-semibold">${formatCurrency(mainTotal)}</span>`;
  ...
  Array.from(document.querySelectorAll('#supplementary-insured-container .person-container')).forEach((cont, idx) => {
    ...
    const total = fee + ((mdp3SelId === id) ? mdp3Fee : 0);
...
// ===== Frequency panel cleanup & zero-hide =====
(function(){
  const $ = (sel, root=document) => root.querySelector(sel);
  function parseNum(t){ return parseInt(String(t||'').replace(/[^\d]/g,''),10)||0; }
  function cleanupFrequencyPanel(){
    ...
    ['freq-main-plus-extra','freq-supp-total','freq-total-period','freq-total-year','freq-diff'].forEach(id => {
      ...
      if (!row) return;
      row.classList.toggle('hidden', parseNum(el.textContent) === 0);
    });
  }
  const __oldUpdate = window.updateSummaryUI;
  window.updateSummaryUI = function(){
    __oldUpdate && __oldUpdate.apply(this, arguments);
  ...
  // first run
  setTimeout(() => { try { cleanupFrequencyPanel(); } catch(e){} }, 0);
})();
