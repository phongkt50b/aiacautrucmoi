...
(function() {
  ...
  function updatePeriodBreakdown() {
    ...
    let suppPeriod;
    if (sel.value === "half") {
      suppPeriod = Math.round((suppAll/1000 * 1.02 / 2))*1000;
    } else if (sel.value === "quarter") {
      suppPeriod = Math.round((suppAll/1000 * 1.04 / 4))*1000;
    } else {
      suppPeriod = suppAll;
    }
    ...
    const setAndHide = (id, val) => {
      const el = $$(id);
      if (!el) return;
      const row = el.closest("div");
      if (row) row.classList.toggle("hidden", Math.max(0, Math.round(val)) === 0);
      const target = (Math.max(0, Math.round(val))).toLocaleString("vi-VN") + " VNĐ";
      if (el.textContent !== target) el.textContent = target;
    };

    setAndHide("#freq-main-plus-extra", mainExtraPeriod);
    setAndHide("#freq-supp-total", suppPeriod);
    setAndHide("#freq-total-period", totalPeriod);
    setAndHide("#freq-total-year", toYear);

    const diffEl = $$("#freq-diff");
    if (diffEl) {
      const row = diffEl.closest("div");
      if (row) row.classList.toggle("hidden", sel.value === "year" || diff === 0);
      const target = (diff).toLocaleString("vi-VN") + " VNĐ";
      if (diffEl.textContent !== target) diffEl.textContent = target;
    }
  } = computeYearTotals();
  ...
  function setupSuppAccordion() {
    ...
    btn.addEventListener("click", (e) => {
      ...
      btn.textContent = "Xem từng người"; // giữ cố định
    });
  });
  }
  ...
  function setupObservers() {
    ...
    const schedule = () => {
      if (rafId) return;
      rafId = requestAnimationFrame(() => { rafId = null; refreshUI(); });
    };
...
// === OVERRIDE generateSummaryTable (chuẩn đặc tả) ===
function generateSummaryTable() {
  const modal = document.getElementById('summary-modal');
  const container = document.getElementById('summary-content-container');
  container.innerHTML = '';
  ...
  try {
    ...
    else if (mainProduct === 'AN_BINH_UU_VIET') paymentTerm = parseInt(document.getElementById('abuv-term')?.value || '0', 10) || 0;
    else paymentTerm = parseInt(document.getElementById('payment-term')?.value || '0', 10) || 0;

    // Kỳ đóng phí
    const freqEl = document.getElementById('payment-frequency');
    const freq = freqEl ? freqEl.value : 'year';
    ...
    const mainStbh = (mainProduct === 'TRON_TAM_AN') ? 100_000_000 : parseFormattedNumber(document.getElementById('main-stbh')?.value);
    ...
    const baseMainAnnual = calculateMainPremium(mainInfo);
    const extraAnnual = getExtraPremiumValue();

    // người bổ sung
    const suppPersons = Array.from(document.querySelectorAll('.person-container'))
      .filter(x => x.id !== 'main-person-container')
      .map(x => getCustomerInfo(x, false));

    // MDP3
    let mdpSel = null, mdpAnnual = 0;
    if (document.getElementById('mdp3-enable')?.checked && window.MDP3){
      mdpSel   = window.MDP3.getSelectedId && window.MDP3.getSelectedId();
      mdpAnnual = Number(window.MDP3.getPremium ? (window.MDP3.getPremium()||0) : 0);
    }

    const calcSuppAnnual = (person, cont) => {
      if (!cont) return 0;
      let s = 0;
      s += calculateHealthSclPremium(person, cont, person.age);
      s += calculateBhnPremium(person, cont, person.age);
      s += calculateAccidentPremium(person, cont, person.age);
      s += calculateHospitalSupportPremium(person, calculateMainPremium(mainInfo), cont, 0, person.age);
      return Math.floor((s * suppK)/1000)*1000;
    };

    const buildSuppRows = (person, cont, includeMdp3) => {
      let html = '';
      ...
      if (scl && scl.querySelector('.health-scl-checkbox')?.checked) {
        ...
        const stbh = getHealthSclStbhByProgram ? getHealthSclStbhByProgram(program) : 0;
        const fee  = Math.floor((calculateHealthSclPremium(person, cont, person.age)*suppK)/1000)*1000;
        const yrs  = Math.max(0, Math.min(targetAge, 75) - person.age + 1);
        html += `<tr><td class="p-2 border"></td><td class="p-2 border">Sức khoẻ Bùng Gia Lực ${labelProgram?'- '+labelProgram:''}</td><td class="p-2 border text-right">${formatCurrency(stbh)}</td><td class="p-2 border text-center">${yrs}</td><td class="p-2 border text-right">${fee?formatCurrency(fee):''}</td></tr>`;
      }
      const bhn = cont?.querySelector('.bhn-section');
      if (bhn && bhn.querySelector('.bhn-checkbox')?.checked) {
        const stbh = parseFormattedNumber(bhn.querySelector('.bhn-stbh')?.value);
        const fee  = Math.floor((calculateBhnPremium(person, cont, person.age)*suppK)/1000)*1000;
        const yrs  = Math.max(0, Math.min(targetAge, 85) - person.age + 1);
        html += `<tr><td class="p-2 border"></td><td class="p-2 border">Bệnh hiểm nghèo 2.0</td><td class="p-2 border text-right">${formatCurrency(stbh)}</td><td class="p-2 border text-center">${yrs}</td><td class="p-2 border text-right">${fee?formatCurrency(fee):''}</td></tr>`;
      }
      const acc = cont?.querySelector('.accident-section');
      if (acc && acc.querySelector('.accident-checkbox')?.checked) {
        const stbh = parseFormattedNumber(acc.querySelector('.accident-stbh')?.value);
        const fee  = Math.floor((calculateAccidentPremium(person, cont, person.age)*suppK)/1000)*1000;
        const yrs  = Math.max(0, Math.min(targetAge, 65) - person.age + 1);
        html += `<tr><td class="p-2 border"></td><td class="p-2 border">Bảo hiểm Tai nạn</td><td class="p-2 border text-right">${formatCurrency(stbh)}</td><td class="p-2 border text-center">${yrs}</td><td class="p-2 border text-right">${fee?formatCurrency(fee):''}</td></tr>`;
      }
      const hs = cont?.querySelector('.hospital-support-section');
      if (hs && hs.querySelector('.hospital-support-checkbox')?.checked) {
        const stbh = parseFormattedNumber(hs.querySelector('.hospital-support-stbh')?.value);
        const fee  = Math.floor((calculateHospitalSupportPremium(person, calculateMainPremium(mainInfo), cont, 0, person.age)*suppK)/1000)*1000;
        const yrs  = Math.max(0, Math.min(targetAge, 65) - person.age + 1);
        html += `<tr><td class="p-2 border"></td><td class="p-2 border">Hỗ trợ chi phí nằm viện (đ/ngày)</td><td class="p-2 border text-right">${formatCurrency(stbh)}</td><td class="p-2 border text-center">${yrs}</td><td class="p-2 border text-right">${fee?formatCurrency(fee):''}</td></tr>`;
      }
      if (includeMdp3 && mdpAnnual>0) {
        // STBH cho MDP3: dùng tổng phí năm của người đó như base biểu thị
        const stbhBase = (person.container?.id === 'main-person-container')
          ? (baseMainAnnual + extraAnnual + calcSuppAnnual(person, cont))
          : (calcSuppAnnual(person, cont));
        const feeAdj = Math.floor((mdpAnnual*suppK)/1000)*1000;
        const yrs    = Math.max(0, Math.min(targetAge, 65) - person.age + 1);
        html += `<tr><td class="p-2 border"></td><td class="p-2 border">Miễn đóng phí 3.0</td><td class="p-2 border text-right">${formatCurrency(stbhBase)}</td><td class="p-2 border text-center">${yrs}</td><td class="p-2 border text-right">${formatCurrency(feeAdj)}</td></tr>`;
      }
      return html;
    };
    ...
    let html1 = `<div class="mb-4">
      ...
      <div class="text-xs italic text-gray-500 mb-2">Các khoản phí dưới đây đã tính theo kỳ đang chọn (Năm/Nửa năm/Quý). Chỉ phí bổ sung áp dụng hệ số theo kỳ.</div>
    ...
    const mainHeaderTotal = baseMainAnnual + extraAnnual + mainSuppNow + ((mdpSel==='main-person-container')? Math.floor((mdpAnnual*suppK)/1000)*1000 : 0);
    ...
    html1 += `<tr>
      ...
      <td class="p-2 border text-right">${baseMainAnnual?formatCurrency(baseMainAnnual):''}</td>
    </tr>`;
    ...
    html1 += buildSuppRows(mainInfo, mainSuppCont, mdpSel==='main-person-container');

    // Những người bổ sung
    suppPersons.forEach(p => {
      const cont = p.container.querySelector('.supplementary-products-container');
      const pSuppNow = calcSuppAnnual(p, cont);
      const pHeaderTotal = pSuppNow + ((mdpSel===p.container.id)? Math.floor((mdpAnnual*suppK)/1000)*1000 : 0);
      html1 += `<tr class="bg-gray-50">
      ...
      html1 += buildSuppRows(p, cont, mdpSel===p.container.id);
    });

    if (mdpSel==='other' && mdpAnnual>0){
      let baseShow = baseMainAnnual + extraAnnual + mainSuppNow;
      html1 += `<tr>
        <td class="p-2 border">Người khác</td>
        <td class="p-2 border">Miễn đóng phí 3.0</td>
        <td class="p-2 border text-right">${formatCurrency(baseShow)}</td>
        <td class="p-2 border text-center">—</td>
        <td class="p-2 border text-right">${formatCurrency(Math.floor((mdpAnnual*suppK)/1000)*1000)}</td>
      </tr>`;
    }
    ...
    for (let i=0; mainInfo.age+i<=targetAge; i++){
      ...
      let suppMainY = 0;
      const mCont = document.querySelector('#main-supp-container .supplementary-products-container');
      if (mCont){
        suppMainY += calculateHealthSclPremium({...mainInfo, age}, mCont, age);
        suppMainY += calculateBhnPremium({...mainInfo, age}, mCont, age);
        suppMainY += calculateAccidentPremium({...mainInfo, age}, mCont, age);
        suppMainY += calculateHospitalSupportPremium({...mainInfo, age}, baseMainAnnual, mCont, 0, age);
      }
      ...
      const suppEachY = suppPersons.map(p=>{
        ...
        if (cont){
          const ageP = p.age + i;
          s += calculateHealthSclPremium({...p, age:ageP}, cont, ageP);
          s += calculateBhnPremium({...p, age:ageP}, cont, ageP);
          s += calculateAccidentPremium({...p, age:ageP}, cont, ageP);
          s += calculateHospitalSupportPremium({...p, age:ageP}, baseMainAnnual, cont, 0, ageP);
        }
        return s;
      });

      if (mdpAnnual>0 && mdpSel){
        if (mdpSel==='main-person-container') suppMainY += mdpAnnual;
        else if (mdpSel!=='other'){
          const idx = suppPersons.findIndex(x => x.container?.id === mdpSel);
          if (idx>=0) suppEachY[idx] += mdpAnnual;
        }
      }
