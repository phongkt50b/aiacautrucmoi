    ...
    try{
      ...
      if(freqInfo.periods === 1){
        ...
        if (freqInfo.breakdown.perMain > 0) {
          freqHtml += `<div>Phí sản phẩm chính:</div><div class="text-right">${formatCurrency(freqInfo.breakdown.perMain)}</div>`;
        }
        if (freqInfo.breakdown.perExtra > 0) {
          freqHtml += `<div>Phí đóng thêm:</div><div class="text-right">${formatCurrency(freqInfo.breakdown.perExtra)}</div>`;
        }
        if (freqInfo.breakdown.perSupp > 0) {
          freqHtml += `<div>Phí sản phẩm bổ sung:</div><div class="text-right">${formatCurrency(freqInfo.breakdown.perSupp)}</div>`;
        }
        freqHtml += `<div class="font-semibold">Tổng:</div><div class="font-semibold text-right">${formatCurrency(freqInfo.perPeriod)}</div>`;
        freqHtml += `<div>Tổng năm:</div><div class="text-right">${formatCurrency(freqInfo.totalYearFromPeriod)}</div>`;
        if (freqInfo.diff > 0) {
          freqHtml += `<div>Chênh lệch:</div><div class="text-right text-red-600">${formatCurrency(freqInfo.diff)}</div>`;
        }
        freqHtml += `</div>`;
...
(function(){
  ...
  function render(){
    ...
    if(suppListEl){
      ...
      // Main person line
      try{
        const mainInfo2 = getCustomerInfo(document.getElementById('main-person-container'), true);
        let mainSupp2 = Number((personFees['main-person-container']?.supp)||0);
        if(mdp3SelectedId==='main-person-container') mainSupp2 += mdp3Fee;
        const rowMain = document.createElement('div');
        rowMain.className = 'flex justify-between items-center py-1 text-sm';
        rowMain.innerHTML = `<span>${sanitizeHtml(mainInfo2.name||'NĐBH chính')}</span><span class="font-semibold">${fmt(mainSupp2)}</span>`;
        suppListEl.appendChild(rowMain);
      }catch(_){}
      ...
      document.querySelectorAll('.person-container').forEach(cont=>{
        ...
        if(mdp3SelectedId && mdp3SelectedId===cont.id) supp += mdp3Fee;
        const row = document.createElement('div');
        row.className = 'flex justify-between items-center py-1 text-sm';
        row.innerHTML = `<span>${sanitizeHtml(info.name || 'NĐBH bổ sung')}</span><span class="font-semibold">${fmt(supp)}</span>`;
        suppListEl.appendChild(row);
      });
      // Không hiển thị hàng 'Người khác' theo yêu cầu
    }
    ...
    try{
      ...
      (function(){
        try{
          ...
          const factor = (periods===2) ? 1.02 : (periods===4 ? 1.04 : 1.0);

          // Tính bổ sung hiện tại (năm đầu) cho NĐBH chính
          const mainSuppCont = document.querySelector('#main-supp-container .supplementary-products-container');
          let suppMainNow = 0;
          if(mainSuppCont){
            suppMainNow += calculateHealthSclPremium(mainInfo, mainSuppCont, mainInfo.age);
            suppMainNow += calculateBhnPremium(mainInfo, mainSuppCont, mainInfo.age);
            suppMainNow += calculateAccidentPremium(mainInfo, mainSuppCont, mainInfo.age);
            suppMainNow += calculateHospitalSupportPremium(mainInfo, baseAnnual, mainSuppCont, 0, mainInfo.age);
          }

          // Bổ sung theo từng NĐBH phụ (năm đầu)
          const suppPersonsNow = [];
          document.querySelectorAll('#supplementary-insured-container .person-container').forEach(cont => {
            const p = getCustomerInfo(cont, false);
            const c = cont.querySelector('.supplementary-products-container');
            let s = 0;
            if(c){
              s += calculateHealthSclPremium(p, c, p.age);
              s += calculateBhnPremium(p, c, p.age);
              s += calculateAccidentPremium(p, c, p.age);
              s += calculateHospitalSupportPremium(p, baseAnnual, c, 0, p.age);
            }
            suppPersonsNow.push({info:p, cont:c, supp:s});
          });
          ...
          try{
            if(window.MDP3){
              mdpSel = (window.MDP3.getSelectedId && window.MDP3.getSelectedId()) || (document.getElementById('mdp3-person-select')?.value||null);
              mdpFee = Number(window.MDP3.getPremium ? window.MDP3.getPremium()||0 : 0);
            }
          }catch(_){}

          // Áp dụng vào tổng bổ sung theo người
          if (mdpFee>0) {
            if (mdpSel==='main-person-container') suppMainNow += mdpFee;
            else if (mdpSel && mdpSel!=='other'){
              const idx = suppPersonsNow.findIndex(x => x.info?.container?.id === mdpSel || x.info?.container === document.getElementById(mdpSel));
              if (idx>=0) suppPersonsNow[idx].supp += mdpFee;
            }
          }

          // Quy đổi theo kỳ: chỉ phí bổ sung áp dụng hệ số
          const perMainExtra = (periods===1) ? (baseAnnual + extraAnnual) : Math.floor((baseAnnual + extraAnnual)/periods);
          const perSuppMain  = (periods===1) ? suppMainNow : Math.round((suppMainNow/1000 * factor / periods))*1000;
          const perPersonTotals = suppPersonsNow.map(x => (periods===1) ? x.supp : Math.round((x.supp/1000 * factor / periods))*1000);
          const totalMainYearFromPer = (perMainExtra + perSuppMain) * periods;

          // Render bảng tóm tắt
          html += `<div class="mb-3"><div class="text-lg font-semibold mb-1">Phần 1 · Tóm tắt sản phẩm</div><div class="text-xs italic text-gray-500">Các khoản phí được tính theo kỳ đang chọn (Năm/Nửa năm/Quý). Phí bổ sung áp dụng hệ số theo kỳ.</div></div>`;
          ...
          // Dòng tổng NĐBH chính
          html += `<tr class="bg-gray-50">
            <td class="p-2 border font-semibold">${sanitizeHtml(mainInfo.name||'NĐBH chính')}</td>
            <td class="p-2 border">—</td><td class="p-2 border text-right">—</td><td class="p-2 border text-center">—</td>
            <td class="p-2 border text-right font-bold">${fmt(totalMainYearFromPer)}</td>
          </tr>`;

          // Dòng sản phẩm chính
          const mainLabel = getProductLabel ? getProductLabel(mainInfo.mainProduct) : mainInfo.mainProduct;
          const stbhMain = (mainInfo.mainProduct==='TRON_TAM_AN') ? 100000000 : parseFormattedNumber(document.getElementById('main-stbh')?.value||'0');
          html += `<tr>
            <td class="p-2 border"></td>
            <td class="p-2 border">${sanitizeHtml(mainLabel)}</td>
            <td class="p-2 border text-right">${fmt(stbhMain)}</td>
            <td class="p-2 border text-center">${paymentTerm||'—'}</td>
            <td class="p-2 border text-right">${baseAnnual>0?fmt(baseAnnual):''}</td>
          </tr>`;
          ...
          if (extraAnnual>0){
            html += `<tr>
              ...
              <td class="p-2 border text-center">${paymentTerm||'—'}</td>
              <td class="p-2 border text-right">${fmt(extraAnnual)}</td>
            </tr>`;
          }

          // Các dòng bổ sung chính
          const addRow = (label, stbh, yrs, fee) => {
            if(fee<=0) return;
            html += `<tr>
              <td class="p-2 border"></td>
              <td class="p-2 border">${sanitizeHtml(label)}</td>
              <td class="p-2 border text-right">${stbh?fmt(stbh):'—'}</td>
              <td class="p-2 border text-center">${yrs||'—'}</td>
              <td class="p-2 border text-right">${fmt(fee)}</td>
            </tr>`;
          };

          const sclChk = mainSuppCont?.querySelector('.health-scl-checkbox');
          if (sclChk?.checked){
            const program = mainSuppCont.querySelector('.health-scl-program')?.value||'';
            ...
            const fee  = (periods===1)? calculateHealthSclPremium(mainInfo, mainSuppCont, mainInfo.age)
                                       : Math.round((calculateHealthSclPremium(mainInfo, mainSuppCont, mainInfo.age)/1000 * factor / periods))*1000;
            const yrs  = Math.max(0, Math.min(targetAge, 75) - mainInfo.age + 1);
            addRow(`Sức khoẻ Bùng Gia Lực${labelProgram?(' - '+labelProgram):''}`, stbh, yrs, fee);
          }
          const bhnChk = mainSuppCont?.querySelector('.bhn-checkbox');
          if (bhnChk?.checked){
            const stbh = parseFormattedNumber(mainSuppCont.querySelector('.bhn-stbh')?.value||'0');
            const fee  = (periods===1)? calculateBhnPremium(mainInfo, mainSuppCont, mainInfo.age)
                                       : Math.round((calculateBhnPremium(mainInfo, mainSuppCont, mainInfo.age)/1000 * factor / periods))*1000;
            const yrs  = Math.max(0, Math.min(targetAge, 85) - mainInfo.age + 1);
            addRow('Bệnh hiểm nghèo 2.0', stbh, yrs, fee);
          }
          const accChk = mainSuppCont?.querySelector('.accident-checkbox');
          if (accChk?.checked){
            const stbh = parseFormattedNumber(mainSuppCont.querySelector('.accident-stbh')?.value||'0');
            const fee  = (periods===1)? calculateAccidentPremium(mainInfo, mainSuppCont, mainInfo.age)
                                       : Math.round((calculateAccidentPremium(mainInfo, mainSuppCont, mainInfo.age)/1000 * factor / periods))*1000;
            const yrs  = Math.max(0, Math.min(targetAge, 65) - mainInfo.age + 1);
            addRow('Bảo hiểm Tai nạn', stbh, yrs, fee);
          }
          const hsChk = mainSuppCont?.querySelector('.hospital-support-checkbox');
          if (hsChk?.checked){
            const stbh = parseFormattedNumber(mainSuppCont.querySelector('.hospital-support-stbh')?.value||'0');
            const fee  = (periods===1)? calculateHospitalSupportPremium(mainInfo, baseAnnual, mainSuppCont, 0, mainInfo.age)
                                       : Math.round((calculateHospitalSupportPremium(mainInfo, baseAnnual, mainSuppCont, 0, mainInfo.age)/1000 * factor / periods))*1000;
            const yrs  = Math.max(0, Math.min(targetAge, 65) - mainInfo.age + 1);
            addRow('Hỗ trợ chi phí nằm viện (đ/ngày)', stbh, yrs, fee);
          }

          // MDP3 nếu gán cho NĐBH chính — STBH hiển thị tổng phí được miễn (ước tính)
          if (mdpFee>0 && mdpSel==='main-person-container'){
            const mdpStbh = baseAnnual + extraAnnual + suppMainNow; // tổng phí được miễn
            const fee  = (periods===1)? mdpFee : Math.round((mdpFee/1000 * factor / periods))*1000;
            const yrs  = Math.max(0, Math.min(targetAge, 65) - mainInfo.age + 1);
            addRow('Miễn đóng phí 3.0', mdpStbh, yrs, fee);
          }

          // --- Người được bảo hiểm bổ sung ---
          suppPersonsNow.forEach(entry => {
            const p = entry.info, cont = entry.info.container, c = entry.cont, sNow = entry.supp;
            const perSupp = (periods===1)? sNow : Math.round((sNow/1000 * factor / periods))*1000;
            const totalYear = ((periods===1)? (baseAnnual+extraAnnual) : Math.floor((baseAnnual+extraAnnual)/periods)) + perSupp;
            html += `<tr class="bg-gray-50">
              <td class="p-2 border font-semibold">${sanitizeHtml(p.name || 'NĐBH bổ sung')}</td>
              <td class="p-2 border">—</td><td class="p-2 border text-right">—</td><td class="p-2 border text-center">—</td>
              <td class="p-2 border text-right font-bold">${fmt(totalYear*periods)}</td>
            </tr>`;

            const addRowP = (label, stbh, yrs, fee) => {
              if(fee<=0) return;
              html += `<tr>
                <td class="p-2 border"></td>
                <td class="p-2 border">${sanitizeHtml(label)}</td>
            ...
            if (c){
              if (c.querySelector('.health-scl-checkbox')?.checked){
                const pg = c.querySelector('.health-scl-program')?.value||'';
                const lb = ({co_ban:'Cơ bản',nang_cao:'Nâng cao',toan_dien:'Toàn diện',hoan_hao:'Hoàn hảo'})[pg]||'';
                const stbh = window.getHealthSclStbhByProgram?window.getHealthSclStbhByProgram(pg):0;
                const fee  = (periods===1)? calculateHealthSclPremium(p, c, p.age) : Math.round((calculateHealthSclPremium(p, c, p.age)/1000 * factor / periods))*1000;
                const yrs  = Math.max(0, Math.min(targetAge, 75) - p.age + 1);
                addRowP(`Sức khoẻ Bùng Gia Lực${lb?(' - '+lb):''}`, stbh, yrs, fee);
              }
              if (c.querySelector('.bhn-checkbox')?.checked){
                const stbh = parseFormattedNumber(c.querySelector('.bhn-stbh')?.value||'0');
                const fee  = (periods===1)? calculateBhnPremium(p, c, p.age) : Math.round((calculateBhnPremium(p, c, p.age)/1000 * factor / periods))*1000;
                const yrs  = Math.max(0, Math.min(targetAge, 85) - p.age + 1);
                addRowP('Bệnh hiểm nghèo 2.0', stbh, yrs, fee);
              }
              if (c.querySelector('.accident-checkbox')?.checked){
                const stbh = parseFormattedNumber(c.querySelector('.accident-stbh')?.value||'0');
                const fee  = (periods===1)? calculateAccidentPremium(p, c, p.age) : Math.round((calculateAccidentPremium(p, c, p.age)/1000 * factor / periods))*1000;
                const yrs  = Math.max(0, Math.min(targetAge, 65) - p.age + 1);
                addRowP('Bảo hiểm Tai nạn', stbh, yrs, fee);
              }
              if (c.querySelector('.hospital-support-checkbox')?.checked){
                const stbh = parseFormattedNumber(c.querySelector('.hospital-support-stbh')?.value||'0');
                const fee  = (periods===1)? calculateHospitalSupportPremium(p, baseAnnual, c, 0, p.age) : Math.round((calculateHospitalSupportPremium(p, baseAnnual, c, 0, p.age)/1000 * factor / periods))*1000;
                const yrs  = Math.max(0, Math.min(targetAge, 65) - p.age + 1);
                addRowP('Hỗ trợ chi phí nằm viện (đ/ngày)', stbh, yrs, fee);
              }
            }

            // MDP3 nếu gán cho người này
            if (mdpFee>0 && mdpSel===p.container?.id){
              const mdpStbh = (baseAnnual + extraAnnual + sNow);
              const fee = (periods===1)? mdpFee : Math.round((mdpFee/1000 * factor / periods))*1000;
              const yrs = Math.max(0, Math.min(targetAge, 65) - p.age + 1);
              addRowP('Miễn đóng phí 3.0', mdpStbh, yrs, fee);
            }
          });

          html += `</tbody></table>`;
        }catch(e){
          console.warn('Summary Part 1 build warn', e);
        }
      })();
      ...
      if (periods>1) html += `<th class=\"p-2 border\">Chênh lệch so với năm</th>`;
      html += `</tr></thead><tbody>`;
      ...
      for(let i=0; (mainInfo.age + i) <= targetAge; i++){
        ...
        html += `<td class="p-2 border text-right">${mainThisYear>0?fmt(mainThisYear):""}</td>`;
        html += `<td class="p-2 border text-right">${extraThisYear>0?fmt(extraThisYear):""}</td>`;
        html += `<td class="p-2 border text-right">${suppMain>0?fmt(suppMain):""}</td>`;
        suppEachArr.forEach(s => { html += `<td class="p-2 border text-right">${s>0?fmt(s):""}</td>`; });
        html += `<td class="p-2 border text-right font-semibold">${fmt(totalFromPeriod)}</td>`;
        if (periods>1) html += `<td class="p-2 border text-right">${diff>0?fmt(diff):""}</td>`;
        html += `</tr>`;
      }
...
(function() {
  ...
  function setupSuppAccordion() {
    ...
    btn.addEventListener("click", (e) => {
      ...
      if (btn.textContent !== FIXED) btn.textContent = FIXED;
    });
