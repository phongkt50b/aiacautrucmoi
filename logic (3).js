...
/* =========================
 * HOTFIX MERGE (stable, non-invasive)
 * - Giữ nội dung & nhãn "Xem từng người" cố định (không đổi khi đổi kỳ đóng phí)
 * - Cộng MDP3 vào đúng người trong danh sách từng người
 * - Xóa dòng "Kỳ: …" & ẩn các dòng bằng 0 trong khối Kỳ đóng phí
 * - Sửa báo cáo generateSummaryTable theo đặc tả (Phần 1 & Phần 2)
 * ========================= */
(function(){
  const $ = (sel, root=document) => root.querySelector(sel);
  ...
  window.formatCurrency = window.formatCurrency || fmt;
  window.roundDownTo1000 = window.roundDownTo1000 || function(n){ return Math.floor((Number(n)||0)/1000)*1000; };
  window.parseFormattedNumber = window.parseFormattedNumber || toNum;

  // 1) Danh sách "Xem từng người" (giữ cố định + cộng MDP3 đúng người)
  window.renderSuppListSimple = function(){
    ...
    wrap.insertAdjacentHTML('beforeend',
      `<div class="flex justify-between items-center py-1 text-sm">
         <span>${sanitizeHtml(mainName)}</span>
         <span class="font-semibold">${formatCurrency(mainSum)}</span>
       </div>`);

    $$('#supplementary-insured-container .person-container').forEach((cont, idx) => {
      const id   = cont.id;
      const name = cont.querySelector('.name-input')?.value?.trim() || \`NĐBH bổ sung \${idx+1}\`;
    ...
    // Nếu MDP3 chọn "Người khác" → thêm dòng riêng để không lệch tổng
    try {
      if (mdp3Id === 'other' && mdp3Fee > 0) {
        wrap.insertAdjacentHTML('beforeend',
          `<div class="flex justify-between items-center py-1 text-sm">
             <span>Miễn đóng phí 3.0 (Người khác)</span>
             <span class="font-semibold">${formatCurrency(mdp3Fee)}</span>
           </div>`);
      }
    } catch{}
  };

  // 2) Nút "Xem từng người" — giữ nhãn cố định, chỉ toggle danh sách
  document.addEventListener('DOMContentLoaded', () => {
    const btn  = $('#toggle-supp-list-btn');
    const list = $('#supp-insured-summaries');
    if (!btn || !list) return;
  ...
  // 3) "Kỳ đóng phí" — bỏ dòng "Kỳ: ..." & ẩn dòng = 0; ẩn cả block nếu chọn Năm
  ...
  (function wrapUpdateSummaryUI(){
    const old = window.updateSummaryUI;
    window.updateSummaryUI = function(){
      try{ old && old.apply(this, arguments); }catch(_){}
      try{ cleanupFrequencyPanel(); }catch(_){}
      try{ window.renderSuppListSimple(); }catch(_){}
    };
  })();
  $('#payment-frequency')?.addEventListener('change', () => setTimeout(() => { cleanupFrequencyPanel(); window.renderSuppListSimple(); }, 0));

  // 4) Báo cáo — generateSummaryTable (đúng đặc tả)
  window.generateSummaryTable = function () {
    ...
    try {
      const targetAge = parseInt($('#target-age-input')?.value || '0', 10);
      const mainCont = $('#main-person-container');
      const mainInfo = window.getCustomerInfo(mainCont, true);
      ...
      const suppPersons = $$('.person-container').filter(x => x.id !== 'main-person-container').map(x => window.getCustomerInfo(x, false));

      const baseMainAnnual = window.calculateMainPremium(mainInfo);
      const extraAnnual = window.getExtraPremiumValue();
      const mainAnnual  = baseMainAnnual + extraAnnual;

      let paymentTerm = 0;
      if (mainProduct === 'TRON_TAM_AN') paymentTerm = 10;
      else if (mainProduct === 'AN_BINH_UU_VIET') paymentTerm = parseInt($('#abuv-term')?.value || '0', 10) || 0;
      else paymentTerm = parseInt($('#payment-term')?.value || '0', 10) || 0;

      if (['PUL_TRON_DOI','PUL_5_NAM','PUL_15_NAM','KHOE_BINH_AN','VUNG_TUONG_LAI'].includes(mainProduct)) {
        const minAge = mainInfo.age + paymentTerm - 1;
        if (targetAge < minAge) throw new Error(`Độ tuổi mục tiêu phải ≥ ${minAge} với sản phẩm hiện tại.`);
      }

      const freq  = $('#payment-frequency')?.value || 'year';
      const per   = (freq==='half')?2:((freq==='quarter')?4:1);
      const suppK = (freq==='half')?1.02:((freq==='quarter')?1.04:1.0);

      const productLabel = window.getProductLabel(mainProduct);
      const mainStbh = (mainProduct === 'TRON_TAM_AN') ? 100_000_000 : toNum($('#main-stbh')?.value);
      const mainTerm = (mainProduct === 'TRON_TAM_AN') ? 10 : (mainProduct === 'AN_BINH_UU_VIET' ? parseInt($('#abuv-term')?.value||'0',10) : parseInt($('#payment-term')?.value||'0',10));

      let mdp3Id = null, mdp3Annual = 0;
      if ($('#mdp3-enable')?.checked && window.MDP3) {
        mdp3Id   = window.MDP3.getSelectedId && window.MDP3.getSelectedId();
      ...
      function personSuppAnnual(person, cont){
        if (!cont) return 0;
        let s = 0;
        s += window.calculateHealthSclPremium(person, cont, person.age);
        s += window.calculateBhnPremium(person, cont, person.age);
        s += window.calculateAccidentPremium(person, cont, person.age);
        s += window.calculateHospitalSupportPremium(person, window.calculateMainPremium(mainInfo), cont, 0, person.age);
        return window.roundDownTo1000(s * suppK);
      }
      ...
      function suppRows(person, cont, includeMdp3Here){
        ...
        if (scl && scl.querySelector('.health-scl-checkbox')?.checked) {
          ...
          const fee  = window.roundDownTo1000(window.calculateHealthSclPremium(person, cont, person.age) * suppK);
          const yrs  = Math.max(0, Math.min(targetAge, 75) - person.age + 1);
          html += `<tr><td class="p-2 border"></td><td class="p-2 border">Sức khoẻ Bùng Gia Lực ${labelProgram?'- '+labelProgram:''}</td><td class="p-2 border text-right">${fmt(stbh)}</td><td class="p-2 border text-center">${yrs}</td><td class="p-2 border text-right">${fee?fmt(fee):''}</td></tr>`;
        }

        const bhn = cont?.querySelector('.bhn-section');
        if (bhn && bhn.querySelector('.bhn-checkbox')?.checked) {
          const stbh = toNum(bhn.querySelector('.bhn-stbh')?.value);
          const fee  = window.roundDownTo1000(window.calculateBhnPremium(person, cont, person.age) * suppK);
          const yrs  = Math.max(0, Math.min(targetAge, 85) - person.age + 1);
          html += `<tr><td class="p-2 border"></td><td class="p-2 border">Bệnh hiểm nghèo 2.0</td><td class="p-2 border text-right">${fmt(stbh)}</td><td class="p-2 border text-center">${yrs}</td><td class="p-2 border text-right">${fee?fmt(fee):''}</td></tr>`;
        }

        const acc = cont?.querySelector('.accident-section');
        if (acc && acc.querySelector('.accident-checkbox')?.checked) {
          const stbh = toNum(acc.querySelector('.accident-stbh')?.value);
          const fee  = window.roundDownTo1000(window.calculateAccidentPremium(person, cont, person.age) * suppK);
          const yrs  = Math.max(0, Math.min(targetAge, 65) - person.age + 1);
          html += `<tr><td class="p-2 border"></td><td class="p-2 border">Bảo hiểm Tai nạn</td><td class="p-2 border text-right">${fmt(stbh)}</td><td class="p-2 border text-center">${yrs}</td><td class="p-2 border text-right">${fee?fmt(fee):''}</td></tr>`;
        }

        const hs = cont?.querySelector('.hospital-support-section');
        if (hs && hs.querySelector('.hospital-support-checkbox')?.checked) {
          const stbh = toNum(hs.querySelector('.hospital-support-stbh')?.value);
          const fee  = window.roundDownTo1000(window.calculateHospitalSupportPremium(person, window.calculateMainPremium(mainInfo), cont, 0, person.age) * suppK);
          const yrs  = Math.max(0, Math.min(targetAge, 65) - person.age + 1);
          html += `<tr><td class="p-2 border"></td><td class="p-2 border">Hỗ trợ chi phí nằm viện (đ/ngày)</td><td class="p-2 border text-right">${fmt(stbh)}</td><td class="p-2 border text-center">${yrs}</td><td class="p-2 border text-right">${fee?fmt(fee):''}</td></tr>`;
        }
        ...
        if (includeMdp3Here && mdp3Annual>0) {
          ...
          const feeAdj = window.roundDownTo1000(mdp3Annual * suppK);
          const yrs    = Math.max(0, Math.min(targetAge, 65) - person.age + 1);
          html += `<tr><td class="p-2 border"></td><td class="p-2 border">Miễn đóng phí 3.0</td><td class="p-2 border text-right">${fmt(stbhBase)}</td><td class="p-2 border text-center">${yrs}</td><td class="p-2 border text-right">${fmt(feeAdj)}</td></tr>`;
        }
        return html;
      }
      ...
      const mainSuppAdj = personSuppAnnual(mainInfo, mainSuppC);
      const mainHeaderTotal = mainAnnual + mainSuppAdj + ((mdp3Id==='main-person-container') ? window.roundDownTo1000(mdp3Annual*suppK) : 0);
      ...
      html1 += `<tr class="bg-gray-50">
        ...
        <td class="p-2 border text-right font-bold">${fmt(mainHeaderTotal)}</td>
      </tr>`;

      html1 += `<tr>
        <td class="p-2 border"></td>
        <td class="p-2 border">${sanitizeHtml(productLabel)}</td>
        <td class="p-2 border text-right">${mainProduct==='TRON_TAM_AN' ? '100.000.000' : fmt(mainStbh)}</td>
        <td class="p-2 border text-center">${mainTerm || '—'}</td>
        <td class="p-2 border text-right">${baseMainAnnual?fmt(baseMainAnnual):''}</td>
      </tr>`;
      ...
      html1 += suppRows(mainInfo, mainSuppC, mdp3Id==='main-person-container');

      suppPersons.forEach(p => {
        const cont = p.container.querySelector('.supplementary-products-container');
        const pSuppAdj = personSuppAnnual(p, cont);
        const pHeaderTotal = pSuppAdj + ((mdp3Id===p.container.id) ? window.roundDownTo1000(mdp3Annual*suppK) : 0);
        html1 += `<tr class="bg-gray-50">
          <td class="p-2 border font-semibold">${sanitizeHtml(p.name||'NĐBH bổ sung')}</td>
          <td class="p-2 border">—</td><td class="p-2 border text-right">—</td><td class="p-2 border text-center">—</td>
          <td class="p-2 border text-right font-bold">${fmt(pHeaderTotal)}</td>
        </tr>`;
        html1 += suppRows(p, cont, mdp3Id===p.container.id);
      });

      if (mdp3Id==='other' && mdp3Annual>0){
        let stbhBase = 0;
        const pf = window.personFees || {};
        for (const pid in pf) stbhBase += (pf[pid].mainBase||0) + (pf[pid].supp||0);
        html1 += `<tr>
          ...
          <td class="p-2 border">Miễn đóng phí 3.0</td>
          <td class="p-2 border text-right">${fmt(stbhBase)}</td>
          <td class="p-2 border text-center">—</td>
          <td class="p-2 border text-right">${fmt(window.roundDownTo1000(mdp3Annual*suppK))}</td>
        </tr>`;
      }
      ...
      suppPersons.forEach(sp => { html2 += `<th class="p-2 border">Phí bổ sung<br>(${sanitizeHtml(sp.name)})</th>`; });
      ...
      for (let i=0; mainInfo.age+i<=targetAge; i++){
        ...
        const mainBaseY = (year <= paymentTerm) ? baseMainAnnual : 0;
        const extraY    = (year <= paymentTerm) ? extraAnnual : 0;

        let suppMainY = 0;
        const mCont = $('#main-supp-container .supplementary-products-container');
        if (mCont){
          suppMainY += window.calculateHealthSclPremium({...mainInfo, age}, mCont, age);
          suppMainY += window.calculateBhnPremium({...mainInfo, age}, mCont, age);
          suppMainY += window.calculateAccidentPremium({...mainInfo, age}, mCont, age);
          suppMainY += window.calculateHospitalSupportPremium({...mainInfo, age}, baseMainAnnual, mCont, 0, age);
        }

        const suppEachY = suppPersons.map(p=>{
          const cont = p.container.querySelector('.supplementary-products-container');
          let s = 0;
          if (cont){
            const ageP = p.age + i;
            s += window.calculateHealthSclPremium({...p, age:ageP}, cont, ageP);
            s += window.calculateBhnPremium({...p, age:ageP}, cont, ageP);
            s += window.calculateAccidentPremium({...p, age:ageP}, cont, ageP);
            s += window.calculateHospitalSupportPremium({...p, age:ageP}, baseMainAnnual, cont, 0, ageP);
          }
          return s;
        });

        if (mdp3Annual>0 && mdp3Id){
          if (mdp3Id==='main-person-container') suppMainY += mdp3Annual;
          else if (mdp3Id!=='other'){
            const idx = suppPersons.findIndex(x => x.container?.id === mdp3Id);
        ...
        html2 += `<td class="p-2 border text-right font-semibold">${fmt(totalFromPer)}</td>`;
        if (per>1) html2 += `<td class="p-2 border text-right">${fmt(diff)}</td>`;
        html2 += `</tr>`;
      }
  ...
  setTimeout(() => { try{ cleanupFrequencyPanel(); renderSuppListSimple(); }catch(_){ } }, 0);
})();
